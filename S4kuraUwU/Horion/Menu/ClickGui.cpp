...
static constexpr float textSize = 0.8f;  // 主大小
...
static constexpr float crossWidth = 0.6f;
static const MC_Color enabledModuleColor = MC_Color(255, 125, 255);  // 已打開功能顏色
static const MC_Color whiteColor = MC_Color(1.f, 1.f, 1.f);          // 分類字體顏色
static const MC_Color selectedModuleColor = MC_Color(255.f, 100.f, 255.f);
static const MC_Color selectedSettingColor1 = MC_Color(255.f, 100.f, 255.f);
static const MC_Color selectedSettingColor2 = MC_Color(255.f, 100.f, 255.f);
static const MC_Color moduleColor = MC_Color(0.1f, 0.1f, 0.1f);  // 主底色
static const MC_Color lmao = MC_Color(255, 255, 255);
static const MC_Color brightModuleBlendColor = MC_Color(0, 0, 0);  // 分類色條
static const MC_Color SettingColor1 = MC_Color (1, 1, 1);  // not enabled enums
static const MC_Color SettingColor2 = MC_Color (1, 1, 1);              // enabled enums and enums when hovering over with mouse
...
void ClickGui::renderTooltip(std::string* text) {
	...
	DrawUtils::fillRectangle(rectPos, moduleColor, .0f);                  // tooltips //功能講解背景
	DrawUtils::drawRectangle(rectPos, MC_Color(255,125,255), 1.f, 0.5f);  // tooltips //功能講解外框
...
void ClickGui::renderCategory(Category category) {
		...
		for (auto& mod : moduleList) {
			...
			if (allowRender) {
				if (!ourWindow->isInAnimation && !isDragging && rectPos.contains(&mousePos)) {  // Is the Mouse hovering above us?
					...
					DrawUtils::fillRectangle(rectPos, MC_Color(1, 1, 1), 0.6f);  // hovering
					...
					DrawUtils::fillRectangle(rectPos, mod->isEnabled() ? enabledModuleColor : moduleColor, 0.8f);  // background
				}
			}
			...
			if (allowRender)
				...
				DrawUtils::drawText(textPos, &textStr, mod->isEnabled() ? MC_Color(245, 239, 245) : MC_Color(245, 239, 245), textSize);  // 主字體顏色
			...
			// Settings
			{
				...
				if (settings->size() > 2 && allowRender) {
				    ...
				    MC_Color(216, 216, 216), crossWidth, crossSize, !clickMod->isExtended); //+的符號顏色
					...
					if (clickMod->isExtended) {
						...
						for (auto setting : *settings) {
							...
							case ValueType::BOOL_T: {
								...
								DrawUtils::fillRectangle(rectPos, MC_Color(1, 1, 1), 1.f);  // mode
								...
								// Checkbox
								{
									...
									DrawUtils::drawRectangle(boxPos, MC_Color(0, 1, 0), isFocused ? 1 : 0.8f, 0.5f);  // the thing that contains the x
								...
								// Text
								{
									...
									DrawUtils::drawText(textPos, &elTexto, isFocused ? MC_Color(250, 250, 250) : MC_Color(250, 250, 250), textSize);  // 模式字體顏色
									currentYOffset += textHeight + (textPadding * 2);
								}
								break;
							}
							case ValueType::ENUM_T: {
								...
								// Text and background
								{
									...
									// currentXOffset + windowSize->x + paddingRight - (crossSize / 2) - 1.f,
									 //currentYOffset + textPadding + (textHeight / 2)),
									// MC_Color(1, 1, 1), crossWidth, crossSize, !setting->minValue->_bool);
							...
							case ValueType::FLOAT_T: {
								...
								// Text and background
								{
									...
									DrawUtils::drawText(textPos, &elTexto, MC_Color(1.0f, 0.5f, 1.0f), textSize);  // changes the color of slider text eg: intensity
							...
							case ValueType::INT_T: {
								...
								// Slider
								{
									...
									// RENDERs & Logic
									{
										...
										DrawUtils::drawRectangle(rect, MC_Color(245, 0, 255), textSize);  // Slider outline
							...
							default: {
								...
								windowSize->x = fmax(windowSize->x, DrawUtils::getTextWidth(&elTexto, textSize) + 5 /* because we add 5 to text padding*/);
								 ...
								 //DrawUtils::drawText(textPos, &elTexto, MC_Color(1.0f, .0f, 1.0f), textSize); //not needed in velvets case
								currentYOffset += textHeight + (textPadding * 2);
							} break;
							}
						}
	...
	// Draw Category Header
	{
		...
		// Draw category component
		{
			...
			DrawUtils::fillRectangle(rectPos, MC_Color(255,125,255), 1.f);        //header 顏色                                                                           // changes color text eg: COMBAT CHAT RENDER
			...
			//GuiUtils::drawCrossLine(vec2_t(
								...
								//	MC_Color(1, 1, 1), crossWidth, crossSize, !ourWindow->isExtended);
		}
	}
