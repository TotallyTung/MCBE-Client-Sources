#include "CrystalAuraNukkit.h"

CrystalAuraNukkit::CrystalAuraNukkit() : IModule(0x0, Category::COMBAT, "CrystalAuraNukkit (skid by hank)") {
	...
	registerBoolSetting("Silent Ca", &this->spoof, this->spoof);
}
int crystalDelay4 = 0;
int crystalDelay5 = 0;
int crystalDelay6 = 0;

CrystalAuraNukkit::~CrystalAuraNukkit() {
}

const char* CrystalAuraNukkit::getModuleName() {
	return ("CrystalAuraNukkit");
}
static std::vector<C_Entity*> targetList7;

void findEntity4(C_Entity* currentEntity, bool isRegularEntity) {
	static auto CrystalAuraWTAMod2 = moduleMgr->getModule<CrystalAuraNukkit>();
	...
	if (dist < CrystalAuraWTAMod2->range) {
		targetList7.push_back(currentEntity);
	}
}

bool checkTargCollision2(vec3_t* block, C_Entity* ent) {
...
bool checkSurrounded3(C_Entity* ent) {
	...
	for (auto blocks : blockChecks) {
		if (!checkTargCollision2(&blocks->toVec3t(), ent)) {
			return false;
		}
	}
	return true;
...
std::vector<vec3_t*> getGucciPlacement3(C_Entity* ent) {
	vec3_t entPos = ent->getPos()->floor();
	entPos.y -= 1;
	...
	if (blockChecks.empty()) {
		...
		blockChecks.push_back(new vec3_ti(entPos.x - 1, entPos.y+1, entPos.z));
		blockChecks.push_back(new vec3_ti(entPos.x, entPos.y + 1, entPos.z + 2));
		blockChecks.push_back(new vec3_ti(entPos.x, entPos.y + 1, entPos.z - 2));
		blockChecks.push_back(new vec3_ti(entPos.x + 2, entPos.y + 1, entPos.z));
		blockChecks.push_back(new vec3_ti(entPos.x - 2, entPos.y + 1, entPos.z));
		blockChecks.push_back(new vec3_ti(entPos.x+1, entPos.y + 1, entPos.z + 1));
		blockChecks.push_back(new vec3_ti(entPos.x-1, entPos.y + 1, entPos.z - 1));
		blockChecks.push_back(new vec3_ti(entPos.x + 1, entPos.y + 1, entPos.z-1));
		blockChecks.push_back(new vec3_ti(entPos.x - 1, entPos.y + 1, entPos.z+1));
		blockChecks.push_back(new vec3_ti(entPos.x + 2, entPos.y + 1, entPos.z + 2));
		blockChecks.push_back(new vec3_ti(entPos.x - 2, entPos.y + 1, entPos.z - 2));
		blockChecks.push_back(new vec3_ti(entPos.x + 2, entPos.y + 1, entPos.z - 2));
		blockChecks.push_back(new vec3_ti(entPos.x - 2, entPos.y + 1, entPos.z + 2));
	}
	...
	for (auto blocks : blockChecks) {
		...
		if (!checkTargCollision2(&blocks->toVec3t(), ent)) {  // very efficient coding here
...
void CrystalAuraNukkit::onEnable() {
	crystalDelay4 = 0;
	hasPlaced2 = false;
}
vec3_t espPosLower2;
...
void CrystalAuraNukkit::onTick(C_GameMode* gm) {
	...
	g_Data.forEachEntity(findEntity4);
	hitArr2.clear();
	placeArr2.clear();
	if (spoof) {
	...
	if (autoplace)
		if ((crystalDelay4 >= this->delay) && !(targetList7.empty())) {
			crystalDelay4 = 0;
			if (!checkSurrounded3(targetList7[0])) {
				std::vector<vec3_t*> gucciPositions = getGucciPlacement3(targetList7[0]);
	...
	g_Data.forEachEntity([](C_Entity* ent, bool b) {
		...
		int range = moduleMgr->getModule<CrystalAuraNukkit>()->range;
...
void CrystalAuraNukkit::onDisable() {
	crystalDelay4 = 0;
	hasPlaced2 = false;
}
/*
...
//*/
 ...
 void CrystalAuraNukkit::onPreRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (localPlayer != nullptr && GameData::canUseMoveKeys()) {
		...
		if (!hitArr2.empty()) {
			for (auto postt : hitArr2) {
				DrawUtils::setColor(Theme::rFloat, Theme::gFloat, Theme::bFloat,1);
				DrawUtils::drawBox(postt->floor(), vec3_t(floor(postt->x) + 1.f, floor(postt->y) + 2.f, floor(postt->z) + 1.f), 1.f, true);
			}
		}  //
	}
}
