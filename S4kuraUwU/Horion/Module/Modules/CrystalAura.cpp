...
void CrystalAura::onTick(C_GameMode* gm) {
	...
	if (delay == delay2 + 1) {
		if (autoplace || g_Data.getLocalPlayer()->getSelectedItemId() == 637) {  // endcrystal
			for (auto& i : targetList)
				if (multiaura)
					CPlace(gm, &i->getPos()->add(0, -1, 0));  // iを使うことによって複?の?手にCrystalAuraをすることができます
				else
					CPlace(gm, &targetList[0]->getPos()->add(0, -1, 0));  // シ?プ?な単体のターゲットにク?スタ?をおきます。
			if (targetList.empty()) getRenderPositon = vec3_ti(0, 0, 0);    //これは0,0,0に?る場?は??ダ??グしないので、??ダ??グをしないよう、0,0,0にしています
		}
		return;
	}
