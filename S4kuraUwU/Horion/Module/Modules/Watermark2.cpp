#include "Watermark2.h"
#include "../../DrawUtils.h"

Watermark2::Watermark2() : IModule(0, Category::HUD, "Watermark of thee client") {
...
const char* Watermark2::getModuleName() {
	return ("Watermark2");
}

void Watermark2::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	static std::string name = "S4kuraUwU";
#ifdef _DEBUG
	...
	if (bottom) {
		vec4_t rect = vec4_t(
			...
			windowSize.y + margin - textHeight -5.5,
			0.f + margin + fullTextLength + borderPadding * 2,
			windowSize.y + margin -5.5);
