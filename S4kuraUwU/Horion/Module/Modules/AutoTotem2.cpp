#include "AutoTotem2.h"

AutoTotem2::AutoTotem2() : IModule(0, Category::PLAYER, "Automatically puts totems into your offhand") {
	Mode.addEntry(EnumEntry("Vanilla", 0));
	//Mode.addEntry(EnumEntry("Open Inv", 1));
	registerEnumSetting("Mode", &Mode, 0);
}

AutoTotem2::~AutoTotem2() {
}

const char* AutoTotem2::getModuleName() {
	return ("AutoTotem2");
}
const char* AutoTotem2::getModName() {
	if (Mode.selected == 0) {
		return " [Vanilla]";
	} else {
		return " [OpenInv]";
...
void AutoTotem2::onTick(C_GameMode* gm) {
	setOffhand = false;
...
void AutoTotem2::craftingScreenController_tick(C_CraftingScreenController* c) {
