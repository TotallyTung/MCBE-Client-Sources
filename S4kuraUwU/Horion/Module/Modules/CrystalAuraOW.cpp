...
void findEntity16(C_Entity* currentEntity16, bool isRegularEntity) {
	...
	if (currentEntity16->width <= 0.f || currentEntity16->height <= 0.f)  // Don't hit this pesky antibot on 2b2e.org
		return;
...
bool CrystalAuraOW::tryCrystalAuraOW(vec3_t CrystalAuraOW) {
	...
	if (blockLegacy->material->isReplaceable) {
		...
		if (foundCandidate) {
			g_Data.getCGameMode()->buildBlock(&blok, 1, false);
			g_Data.getCGameMode()->buildBlock(&blok, 2, false);
