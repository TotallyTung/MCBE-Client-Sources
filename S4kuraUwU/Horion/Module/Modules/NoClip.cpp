...
NoClip::NoClip() : IModule(0x0, Category::MOVEMENT, "NoClip through walls on all axis") {
	registerFloatSetting("Horizontal Speed", &this->speed, this->speed, 0.5f, 5.f);
	registerFloatSetting("Vertical Speed", &this->upanddown, this->upanddown, 0.5f, 5.f);
...
void NoClip::onDisable() {
	if (g_Data.getLocalPlayer() != nullptr)
		g_Data.getLocalPlayer()->aabb.upper.y = g_Data.getLocalPlayer()->aabb.lower.y + (float)1.5f;
	if (NoPacket)
