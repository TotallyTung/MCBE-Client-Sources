...
void Watermark::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	Utils::ColorConvertRGBtoHSV(rcolors[2], rcolors[1], rcolors[0], currColor[2], currColor[1], currColor[0]);
	currColor[0] += 1.f;
	Utils::ColorConvertHSVtoRGB(currColor[2], currColor[1], currColor[0], currColor[2], currColor[1], currColor[0]);
	...
	if (mode.selected == 0) {
		if (player != nullptr) {
			...
			std::string name1 = "S4kuraUwU1.7.0Beta";
			std::string name2 = "Private";
