...
void Hooks::ClientInstanceScreenModel_sendChatMessage(void* _this, TextHolder* text) {
	...
	if (text->getTextLength() > 0) {
		...
		if (*message == cmdMgr->prefix) {
			...
			if (!helpedUser) {
				helpedUser = true;
				g_Data.getClientInstance()->getGuiData()->displayClientMessageF("%sYour S4kuraUwU prefix is: \"%s%c%s\"", RED, YELLOW, cmdMgr->prefix, RED);
					...
					if (moduleMgr->isInitialized() && shouldRenderArrayList) {
						...
						for (std::set<IModuleContainer>::iterator it = modContainerList.begin(); it != modContainerList.end(); ++it) {
							...
							if (!GameData::canUseMoveKeys() && rectPos.contains(&mousePos) && hudModule->clickToggle) {
								...
								if (leftMouseDown) {
									DrawUtils::fillRectangle(selectedRect, MC_Color(255, 55, 255), textSize);
									if (executeClick)
							...
							if (ThemeMod->rainbow) {
								if (arrayMod->underline) {
									DrawUtils::fillRectangle(underline, MC_Color(currColor), textSize);
								}
								if (arrayMod->outline) {
									DrawUtils::fillRectangle(leftRect, MC_Color(currColor), textSize);
								}
								if (arrayMod->shadow) {
								...
								if (arrayMod->underline) {
									DrawUtils::fillRectangle(underline, MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat), textSize);
								}
								if (arrayMod->outline) {
									DrawUtils::fillRectangle(leftRect, MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat), textSize);
								}
								if (arrayMod->shadow) {
									DrawUtils::drawText(textPos2, &textStr, MC_Color(Theme::rFloat4, Theme::gFloat4, Theme::bFloat4), textSize);
								}
								DrawUtils::drawText(textPos, &textStr, MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat), textSize);
							}
							...
							/* if (arrayMod->bugs) {
								...
								if (arrayMod->ice) {
								DrawUtils::fillRectangle(topIce, MC_Color(209, 237, 242), textSize);
								DrawUtils::fillRectangle(rightRect, MC_Color(220, 220, 220), textSize);
								}
							if (arrayMod->bottom) {
								yOffset -= textHeight + (textPadding * 2);
							} else {
...
void Hooks::GameMode_startDestroyBlock(C_GameMode* _this, vec3_ti* a2, uint8_t face, void* a4, void* a5) {
	...
	/* if (InstaBreak->isEnabled() && (InstaBreak->mineNow)) {
		_this->destroyBlock(&InstaBreak->currentBlock, face);
		return;
	}*/
...
void Hooks::Actor__setRot(C_Entity* _this, vec2_t& angle) {
	...
	if (jtwdBRMod->isEnabled() &&
		jtwdBRMod->rotateBreak.GetSelectedEntry().GetValue() == 1 &&
		_this == g_Data.getLocalPlayer() && !jtwdBRMod->breakArrEmpty) {
		func(_this, angle = jtwdBRMod->rotAngleB);
	}
	if (jtwdCAMod->isEnabled() &&
	...
	if (surrMod->isEnabled() && (surrMod->rotate.GetSelectedEntry().GetValue() == 1 || surrMod->rotate.GetSelectedEntry().GetValue() == 3) &&
		...
		if (surrMod->rotate.GetSelectedEntry().GetValue() == 3) {
			for (int i = 0; i > -90; i--) {
