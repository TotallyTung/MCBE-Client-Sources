...
PacketMine::PacketMine() : Module("PacketMine", "NULL", Category::PLAYER) {
	...
	addSlider<float>("Speed", "NULL", ValueType::FLOAT_T, &packetmultiplier, 0.1, 1.f);
...
std::pair<int, float> PacketMine::getBestPickaxeSlot(Block* block) {
	...
	float destroyRate = gm->getDestroyRate(block) * packetmultiplier;
	for (int i = 0; i < 9; i++) {
		plrInv->selectedSlot = i;
		float value = gm->getDestroyRate(block) * packetmultiplier;
