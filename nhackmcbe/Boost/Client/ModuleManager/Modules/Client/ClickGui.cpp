...
ClickGui::ClickGui() : Module("ClickGui", "A GUI use to enable/disable modules.!!", Category::CLIENT, VK_INSERT) { //GUI MADE BY MEOWNEOCAT
	addEnumSetting("Mode", "NULL", { "Melody", "Old", "NewGen" }, &Mode);
	addSlider<float>("Blur", "NULL", ValueType::FLOAT_T, &blurStrength, 0.f, 20.f);
	addSlider<float>("Transparency", "NULL", ValueType::FLOAT_T, &transparency, 0.f, 255.f);
...
void ClickGui::render(ImDrawList* drawlist) {
	...
	static const float windowWidth = 222.f;
	static const float windowWidth21 = 212.f;
	static const float textSize = 1.f;
	const float textHeight = ImGuiUtils::getTextHeight(textSize);
	static const float textPadding = 1.3f * textSize;

	float roundValue = (Mode == 0) ? 0.0f : 0.f;
	//float roundValue = (Mode == 1) ? 0.0f : 0.f;
	float roundValue2 = (Mode == 0) ? 0.0f : 0.f;

	static const UIColor whiteColor(244, 244, 244, 255);
	static const UIColor whiteColor2(244, 244, 244, 255);
	//UIColor whiteColor2 = colorsMod->getColor(); //for header text. it looks unique
	static const UIColor semiGrayColor(244, 244, 244, 254);
	//UIColor semiGrayColor = colorsMod->getColor(); //swap for color.
	...
	{
		// Render Background
		UIColor bgColor(12, 1, 12, (int)(100 * openDuration));
		if (blurStrength > 0.f) {
			ImFX::Begin(drawlist);
		...
		for (ClickWindow* window : windowList) {
			...
			UIColor cbgColor = colorsMod->getColor();
			//static const UIColor cbgColor(255, 255, 255, 255);
			ImGuiUtils::fillRectangle(cRectPos, cbgColor, 0, ImDrawFlags_RoundCornersTop);
			ImGuiUtils::fillRectangle(Vec4<float>(cRectPos.x, cRectPos.w - textPadding * 3.f, cRectPos.z, cRectPos.w), cbgColor, 0 + transparency);
			ImGuiUtils::fillRectangle(Vec4<float>(cRectPos.x, cRectPos.w - textPadding * 2.f, cRectPos.z, cRectPos.w), cbgColor, 0 + transparency);
			...
			if (window->extended) {
				...
				for (Module* mod : window->modList) {
					...
					static const UIColor mbgColor = UIColor(0, 31, 21, 0 + transparency);
					...
					if (mod->isEnabled()) {
						if (Mode == 0) {
							...
							ImGuiUtils::fillRectangle(mRectPos2, mainColor);
						}
					}


					Vec2<float> mTextPos = { mRectPos.x + textPadding * 4.f, mRectPos.y + textPadding };
					Vec2<float> mTextPos2 = { mRectPos.x + 203 + textPadding * 4.f, mRectPos.y + textPadding };
					ImGuiUtils::drawText(
						mTextPos,
						mod->getModuleName(),
					...
					if (mRectPos2.contains(mousePos)) {
						...
						ImGuiUtils::fillRectangle(mRectPos2, selectedColor);
						...
						for (Setting* setting : *mod->getSettings()) {
							...
							case (SettingType::BOOL): {
								if (setting->valuePtr->_bool) {
									if (Mode == 0) drawlist->AddRectFilledMultiColor(ImVec2(sRectPos.x, sRectPos.y), ImVec2(sRectPos.z, sRectPos.w), mainColor.toImColor(), ImColor(0, 31, 21, 0), ImColor(0, 31, 21, 0), mainColor.toImColor());
	...
	if (this->tooltips && tooltipString != std::string("NULL")) {
		...
		ImGuiUtils::fillRectangle(rectPos, UIColor(21, 0, 0, 255), roundValue);
		ImGuiUtils::drawRectangle(rectPos, UIColor(36, 0, 0, 255), (Mode == 0) ? 1.f : 2.f, roundValue);
	...
	ImGuiUtils::fillRectangle(searchRectPos, UIColor(29, 0, 0, 255), roundValue);
	ImGuiUtils::fillRectangle(typeRectPos, UIColor(21, 0, 0, 255), roundValue);
	...
	if (!isSearching && searchingModule.empty()) ImGuiUtils::drawText(typeTextPos, "Search for Module", UIColor(125, 0, 0, 255), textSize, false);
	lastKeyPress = -1;
}