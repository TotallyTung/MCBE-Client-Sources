...
void Notifications::Render(ImDrawList* drawlist) {
	...
	for (int i = 0; i < notifList.size(); i++) {
		...
		if (notifBox->duration != 0.f) {
			...
			ImGuiUtils::fillRectangle(underline, UIColor(255, 0, 0, (int)(255 * idk)), 0.f);
		}
		ImGuiUtils::drawText(textPos, notifBox->message.c_str(), UIColor(255, 0, 0, (int)(255 * idk)), textSize, false);
