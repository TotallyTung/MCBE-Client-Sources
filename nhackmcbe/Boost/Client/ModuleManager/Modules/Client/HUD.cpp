...
HUD::HUD() : Module("HUD", "Shows Client Hud.", Category::CLIENT) {
	...
	addBoolCheck("Watermark", "NULL", &watermark);
...
void HUD::onImGuiRender(ImDrawList* drawlist) {
	...
	float xPosqw = io.DisplaySize.x - 1.f;
	static UIColor whiteColorw = UIColor(255, 211, 211, 255);
	static UIColor semiGrayColorw = UIColor(200, 155, 155, 255);
	if (watermark) {
		ImGuiUtils::drawText(Vec2<float>(xPosqw - ImGuiUtils::getTextWidth("nhack v0.2", size), 0), "nhack v0.2", whiteColorw, size);
	}
	LocalPlayer* localPlayer = mc.getLocalPlayer();
	if (localPlayer == nullptr) return;
	...
	if (AutoCrystalMod->Crystalcounter) {
		...
		char message[256];
		sprintf(message, "speed : %i", AutoCrystalMod->crystalSpeed);
