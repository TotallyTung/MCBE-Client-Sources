...
void IRC::execute(const std::vector<std::string>& args) {
    if (args.size() < 2) {
        char errorMsg[] = "[%snhack%s] %sInvalid command! Usage: .(IRC,internet,talk) words";
    ...
    case 'i':
        if (subCommand == "irc") {
            ...
            if (sendDiscordMessage(message)) {
                mc.DisplayClientMessage("[%snhack%s] %sMessage sent to Discord webhook successfully!", BLUE, WHITE, GREEN);
            }
            else {
                mc.DisplayClientMessage("[%snhack%s] %sFailed to send message to Discord webhook.", BLUE, WHITE, RED);
            }
        }
        break;
    }
