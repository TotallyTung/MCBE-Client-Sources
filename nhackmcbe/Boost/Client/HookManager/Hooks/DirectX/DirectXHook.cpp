...
void DirectXHook::Render(ImDrawList* drawlist) {
	...
	if (mc.getLocalPlayer() == nullptr && client->isInitialized() && client->moduleMgr->isInitialized()) {
		drawlist->AddText(NULL, 100, ImVec2(12.f, 12.f), IM_COL32(0, 0, 0, 244), "nhack");
		drawlist->AddText(NULL, 100, ImVec2(10.f, 10.f), IM_COL32(255, 0, 0, 255), "nhack");

	}
	//ImGui::Text("%i", mc.getLocalPlayer()->getLevel()->getRuntimeActorList().size());
	//ImGuiUtils::drawText(Vec2<float>(10.f, 6.f), "Boost", UIColor(255, 0, 0, 255), 2.f);
}
