...
void Friend::execute(const std::vector<std::string>& args) {
    ...
    if (args.size() < 2) {
        mc.DisplayClientMessage("[%snuke.triangle%s] %sInvalid command! Usage: .(f,fri,friends) <add/a> <playername> | <delete/d> <playername> | show", DARK_PURPLE, WHITE, RED);
        return;
    }
    ...
    if (subCommand == "add" || subCommand == "a") {
        ...
        if (std::find(TargetUtils::Friend.begin(), TargetUtils::Friend.end(), sanitizedPlayerName) == TargetUtils::Friend.end()) {
            ...
            mc.DisplayClientMessage("[%snuke.triangle%s] %s%s added to friends list!", DARK_PURPLE, WHITE, GREEN, sanitizedPlayerName.c_str());
        }
        else {
            mc.DisplayClientMessage("[%snuke.triangle%s] %s%s is already in friends list!", DARK_PURPLE, WHITE, YELLOW, sanitizedPlayerName.c_str());
        }
    }
        ...
        if (it != TargetUtils::Friend.end()) {
            ...
            mc.DisplayClientMessage("[%snuke.triangle%s] %s%s removed from friends list!", DARK_PURPLE, WHITE, GREEN, sanitizedPlayerName.c_str());
        }
        else {
            mc.DisplayClientMessage("[%snuke.triangle%s] %s%s is not in friends list!", DARK_PURPLE, WHITE, YELLOW, sanitizedPlayerName.c_str());
        }
    }
    else if (subCommand == "show" || subCommand == "s") {
        mc.DisplayClientMessage("[%snuke.triangle%s] %sFriends List:", DARK_PURPLE, WHITE, GREEN);
        ...
        mc.DisplayClientMessage("[%snuke.triangle%s] %sFriends list cleared!", DARK_PURPLE, WHITE, GREEN);
    }
    else {
