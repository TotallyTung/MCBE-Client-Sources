...
public:
	...
	//static void drawBox3d(const vec3_t& lower, const vec3_t& upper, float scale = 1.f, bool onUi = false);
	static void drawLineA(vec2_t start, vec2_t end, float lineWidth, MC_Color col);
	static float deltaTime;
	static void drawBox3d2(const vec3_t& lower, const vec3_t& upper, float scale = 1.f, bool onUi = false);
	static void drawBox3dFilled2(const vec3_t& lower, const vec3_t& upper, float scale = 1.f, bool outline = false, bool onUi = false);
	...
	static inline void drawRectangle2(vec4_t pos, MC_Color col, float alpha, float lineWidth = 1.0f) {
		lineWidth /= 2;
		fillRectangle(vec4_t(pos.x - lineWidth, pos.y - lineWidth, pos.z + lineWidth, pos.y + lineWidth), col, alpha);  // TOP
		fillRectangle(vec4_t(pos.x - lineWidth, pos.y, pos.x + lineWidth, pos.w), col, alpha);                          // LEFT
		fillRectangle(vec4_t(pos.z - lineWidth, pos.y, pos.z + lineWidth, pos.w), col, alpha);                          //
