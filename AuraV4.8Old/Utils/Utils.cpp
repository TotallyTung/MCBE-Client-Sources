...
void Utils::sendCommand(std::string cmd, ...) {
	...
	g_Data.getClientInstance()->loopbackPacketSender->sendToServer(&CRP);
};

std::string Utils::getClientName() {
//	if (moduleMgr->isInitialized()) if (moduleMgr->getModule<Interface>()->theme.getSelectedValue() == 1)
		return "Aura V4";
};
...
void Utils::systemPlay(std::string name) {
	std::string path = getenv("APPDATA") + std::string("\\..\\Local\\Packages\\Microsoft.MinecraftUWP_8wekyb3d8bbwe\\RoamingState\\AuraV4\\Assets\\") + std::string(name);
	PlaySoundA((LPCSTR)path.c_str(), NULL, SND_FILENAME | SND_ASYNC);
}

void Utils::makeFolderInRoaming(std::string file) {
...
void Utils::makeFolderInPacket(std::string file) {
	std::string path = getenv("APPDATA") + std::string("\\..\\Local\\Packages\\Microsoft.MinecraftUWP_8wekyb3d8bbwe\\RoamingState\\AuraV4\\") + std::string(file);
	// Creating File
	if (mkdir(path.c_str()) == -1)
		return;
	else
...
void Utils::makeFolderInAssets(std::string file) {
	std::string path = getenv("APPDATA") + std::string("\\..\\Local\\Packages\\Microsoft.MinecraftUWP_8wekyb3d8bbwe\\RoamingState\\AuraV4\\Assets\\") + std::string(file);
	// Creating File
	if (mkdir(path.c_str()) == -1)
		return;
	else
...
uintptr_t Utils::FindSignatureModule(volatile char* szModule, volatile char* szSignature) {
	volatile char* pattern = szSignature;
	uintptr_t firstMatch = 0;
	static const auto rangeStart = (uintptr_t)GetModuleHandleA((const char*)szModule);
	static MODULEINFO miModInfo;
	static bool init = false;
	if (!init) {
		init = true;
...
#pragma clang diagnostic ignored "-Wunused-variable"
#endif
	volatile char* sig = szSignature;  // Put sig in here to access it in debugger
	// This will not get optimized away because we are in debug
	// Leave this in here to quickly find bad signatures in case of updates
	logF("Signature dead: %s", szSignature);
	if (false) {
