...
//	virtual class AttributeInstance* getAttribute2(class Attribute* Attribute);
...
public:
	...
	bool isInRain() {
		static void* isRainingAt = (void*)FindSignature("48 89 5C 24 ? 57 48 83 EC ? 49 8B D8 48 8B FA E8 ? ? ? ? 84 C0 74 ? 48 8B D3 48 8B CF E8 ? ? ? ? 4C 8B C3 48 8B D7 48 8B C8 E8 ? ? ? ? 0F 2F 05 ? ? ? ? 0F 97 C0");
		vec3_t pos = *this->getPos();
		vec3_ti blockPos = vec3_ti((int)floor(pos.x), (int)floor(pos.y), (int)floor(pos.z));
		bool rain = Utils::CallFunc<bool, Weather*, C_BlockSource*, vec3_ti*>(
			isRainingAt,
			this->getDimension()->weather,
			this->region,
			&blockPos
		);
		return rain;
	...
	Dimension* getDimension() {
		return *reinterpret_cast<Dimension**>(reinterpret_cast<__int64>(this) + 0x360);
	}
	...
	void cancelHurtAnimation() {
		...
		*(int *)((uintptr_t)(this) + 0x740) = 0;*/ // Packet phe vat=))
...
public:
	...
	virtual __int64 openBook(int, bool, int, __int64*);                                                 // 3
