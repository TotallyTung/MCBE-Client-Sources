...
C_MobEquipmentPacket::C_MobEquipmentPacket() {
	static uintptr_t** C_MobEquipmentPacketVtable = 0x0;
	if (C_MobEquipmentPacketVtable == 0x0) {
		uintptr_t sigOffset = FindSignature("48 8D 05 ? ? ? ? 48 89 06 48 8B C6 4C 8D 9C 24");
		int offset = *reinterpret_cast<int*>(sigOffset + 3);
		C_MobEquipmentPacketVtable = reinterpret_cast<uintptr_t**>(sigOffset + offset + /*length of instruction*/ 7);
#ifdef _DEBUG
		if (C_MobEquipmentPacketVtable == 0x0 || sigOffset == 0x0)
			__debugbreak();
#endif
	}
	memset(this, 0, sizeof(C_MobEquipmentPacket));  // Avoid overwriting vtable
	vTable = C_MobEquipmentPacketVtable;
}
...
CommandRequestPacket::CommandRequestPacket(std::string cmd) {
	...
	memset(this, 0, sizeof(CommandRequestPacket));  // Avoid overwriting vtable
	vTable = commandRequestPacketVtable = 0x0;
