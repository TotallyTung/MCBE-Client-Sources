...
#include "../SDK/Tag.h"
#include "pch.h"
#include "../AuraClient/Module/Modules/Other/Background.h"
#include "../AuraClient/Module/Modules/Other/AltTheme.h"
...
void Hooks::Init() {
	...
	// clang-format off
	//MessageBox(0, L"Greetings. This is Afar and thank you for using AuraV4. AuraV4 is a utility mod that can enhance your pvp experience in anarchy servers, such as 2B2TPE. It is for learning and research only. You are responsible for all consequences caused by using this Mod.", L"AuraV4", MB_OK);
	//SetWindowTextA(FindWindowA(NULL,"Minecraft"),"Horion Client Dev");
	...
	// Signatures
	{
		...
		void* LevelRendererPlayerbobHurt = reinterpret_cast<void*>(FindSignature("48 89 5C 24 ?? 57 48 83 EC ?? 48 8B 89 ?? ?? ?? ?? 48 8B FA 0F 29 7C 24"));
		g_Hooks.LevelRendererPlayer__bobHurtHook = make_unique<FuncHook>(LevelRendererPlayerbobHurt, Hooks::LevelRendererPlayer__bobHurt);
...
void Hooks::LevelRendererPlayer__bobHurt(__int64 _this, glm::mat4& matrix, float lerpT) {
	static auto origFunc = g_Hooks.LevelRendererPlayer__bobHurtHook->GetFastcall<void, __int64, glm::mat4&, float>();
	static auto viewmodelMod = moduleMgr->getModule<ViewModel>();
	glm::mat4 View = matrix;
	matrix = View;
	if (viewmodelMod->isEnabled()) {
		// Custom Settings
		if (viewmodelMod->Translate)
			matrix = glm::translate<float>(matrix, glm::vec3(viewmodelMod->trans.x, viewmodelMod->trans.y, viewmodelMod->trans.z)); // X Y Z

		if (viewmodelMod->Scale)
			matrix = glm::scale<float>(matrix, glm::vec3(viewmodelMod->scale.x, viewmodelMod->scale.y, viewmodelMod->scale.z)); // SCALE

		if (viewmodelMod->Rotate)
			matrix = glm::rotate<float>(matrix, viewmodelMod->RotatePos, glm::vec3(viewmodelMod->rotat.x, viewmodelMod->rotat.y, viewmodelMod->rotat.z)); // idk unused
	}
...
void Hooks::Actor_aiStep(C_Entity* _this) {
	static auto Func = g_Hooks.Actor_aiStepHook->GetFastcall<void, C_Entity*>();
	static PacketMine* packetMineMod = moduleMgr->getModule<PacketMine>();
	C_LocalPlayer* C_localPlayer = g_Data.getLocalPlayer();

	if (packetMineMod->isEnabled() && _this == C_localPlayer) {
		//*((char*)_this + 7500) = 0;
		packetMineMod->updateBreak(C_localPlayer, g_Data.getCGameMode());
	}
	Func(_this);
}
...
void Hooks::Actor_rotation(C_Entity* _this, vec2_t& angle) {
	static auto Func = g_Hooks.Actor_rotationHook->GetFastcall<void, C_Entity*, vec2_t&>();
	...
	if (_this == g_Data.getLocalPlayer()) {
		if (killauraMod->isEnabled() && !killauraMod->targetListEmpty && killauraMod->RotMeow) angle = killauraMod->angle;
...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
...

	{
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
			...
			*/
			float wtextWid = DrawUtils::getTextWidth(&string("Aura - V4.8 Beta"), 2.f);
			...
			DrawUtils::drawText(vec2_t(wid.x - wtextWid - 5, wid.y - wid.y + 7), &string("Aura - V4.8 Beta"), MC_Color(interfaceColor), 2.f, 1.f, true);
			DrawUtils::fillRectangle(vec4_t(0, 0, g_Data.getClientInstance()->getGuiData()->widthGame, g_Data.getClientInstance()->getGuiData()->heightGame), MC_Color(interfaceColor), 0.03f);
		...
		std::string screenName2(g_Hooks.currentScreenName);
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
			// Draw BIG epic Aura watermark
			{
				static auto ThemeMod = moduleMgr->getModule<Theme>();
				std::string text = "Aura V4";
				...
				//DrawUtils::drawText(vec2_t(18, 18), &screenName, MC_Color(255, 255, 255), 1, 1);
				DrawUtils::drawText(textPos, &text, MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat, 1.f), 8.f);
				//DrawUtils::drawText(textPos, &text, MC_Color(255, 255, 255), 1.f, 8.f);
