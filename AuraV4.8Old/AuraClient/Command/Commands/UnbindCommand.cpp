...
bool UnbindCommand::execute(std::vector<std::string>* args) {
	...
	if (moduleName == "all") {
		if (args->size() >= 3 && args->at(2) == "force") {
			...
			clientMessageF("[Aura] %sUnbound all modules!", GREEN);
		} else {
			clientMessageF("[Aura] %sAre you sure? This will unbind %sALL%s%s modules!", RED, BOLD, RESET, RED);
			clientMessageF("[Aura] %sUse %s.unbind all force%s to unbind all modules", RED, WHITE, RED);
		}
		return true;
	}
	...
	if (!modOpt.has_value()) {
		clientMessageF("[Aura] %sCould not find module with name: %s", RED, moduleName.c_str());
		return true;
	} 

	auto mod = modOpt.value();
	mod->setKeybind(0x0);
	clientMessageF("[Aura] %sSuccessfully unbound %s!", GREEN, mod->getRawModuleName());
	return true;
}
