...
/*bool SpammerCommand::execute(std::vector<std::string>* args) {
	...
	if (option == "text") {
		...
		clientMessageF("[Aura] %sSpammer message set to %s%s%s!", GREEN, GRAY, text.c_str(), GREEN);
		return true;
	} else if (option == "delay") {
		int delay = assertInt(args->at(2));
		if (delay < 1) {
			clientMessageF("[Aura] %sDelay needs to be 1 or more!", RED);
			return true;
		} else {
		...
		clientMessageF("[Aura] %sBypass set to %s%s%s!", GREEN, GRAY, state ? "true" : "false", GREEN);
		return true;
