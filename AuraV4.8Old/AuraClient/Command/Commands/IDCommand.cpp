	...
	if (args->at(1) == "pointing") {
		...
		if (pointing->hasEntity()) {
			...
			if (entity != nullptr) {
				...
				clientMessageF("[Aura] Entity Name: %s", entityName.c_str());
				clientMessageF("[Aura] Entity ID: %s", entityID.c_str());
			}
		} else {
			clientMessageF("[Aura] Block Name: %s", blockName);
			clientMessageF("[Aura] Block ID: %d", blockID);
		}
		return true;
		...
		if (stack->item != nullptr) {
			...
			g_Data.getGuiData()->displayClientMessageF("[Aura] Item Name: %s", name);
			g_Data.getGuiData()->displayClientMessageF("[Aura] Item ID: %lld", id);
		}
		return true;
	}
	return false;
