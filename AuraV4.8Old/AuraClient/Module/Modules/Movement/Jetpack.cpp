...
Jetpack::Jetpack() : IModule('F', Category::MOVEMENT, "Fly around like you had a Jetpack!") {
	...
	registerBoolSetting("PvP", &pvpig, pvpig);
...
void Jetpack::onEnable() {
	...
	if (pvpig) {
		if (nc->isEnabled()) nc->setEnabled(false);
		moduleMgr->getModule<NoClipGlide>()->setEnabled(false);
...
void Jetpack::onDisable() {
	moduleMgr->getModule<Flight>()->setEnabled(true);
	if (pvpig) {
		moduleMgr->getModule<NoClip>()->setEnabled(true);
		moduleMgr->getModule<NoClipGlide>()->setEnabled(true);
