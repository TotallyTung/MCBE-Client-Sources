...
using namespace std;
Criticals::Criticals() : IModule(0, Category::COMBAT, "Always do critical damages") {
...
const char* Criticals::getModuleName() {
	if (mode.getSelectedValue() == 0)  name = string("Criticals ") + string(GRAY) + string("[") + string(WHITE) + string("Vanilla") + string(GRAY) + string("]");
	if (mode.getSelectedValue() == 1)  name = string("Criticals ") + string(GRAY) + string("[") + string(WHITE) + string("Position") + string(GRAY) + string("]");
	if (mode.getSelectedValue() == 2)  name = string("Criticals ") + string(GRAY) + string("[") + string(WHITE) + string("Jump") + string(GRAY) + string("]");
	if (mode.getSelectedValue() == 3)  name = string("Criticals ") + string(GRAY) + string("[") + string(WHITE) + string("Hive") + string(GRAY) + string("]");
	if (mode.getSelectedValue() == 4)  name = string("Criticals ") + string(GRAY) + string("[") + string(WHITE) + string("Patch") + string(GRAY) + string("]");
	if (mode.getSelectedValue() == 5)  name = string("Criticals ") + string(GRAY) + string("[") + string(WHITE) + string("Nukkit") + string(GRAY) + string("]");
	return name.c_str();
}
...
void Criticals::onTick(C_GameMode* gm) {
	...
	if (mode.getSelectedValue() == 5) {
		if (!player->onGround && !player->isInWater()) {
			player->setPos(player->getPos()->add(0.f, -0.03f, 0.f));
		}
	}
}
