...
IModule::IModule(int key, Category c, const char* tooltip) {
	...
	this->registerBoolSetting(string("Hide"), &this->shouldHide, shouldHide);
	this->registerEnumSetting("Toggle", &togglemod, 0);
...
void IModule::HideSetting(const std::string& name) {
	for (auto& setting : settings) {
		if (strcmp(setting->name, name.c_str()) == 0) {
...
void IModule::ShowSetting(const std::string& name) {
	for (auto& setting : settings) {
		if (strcmp(setting->name, name.c_str()) == 0) {
...
void IModule::onJoiningServer() {
};

bool IModule::isHoldMode() {
	if (this->togglemod.getSelectedValue() == 0) return false;
	else
	{
		return true;
	}
...
void IModule::setEnabled(bool enabled) {
	if (this->enabled != enabled) {
		...
		//static auto notiMod = moduleMgr->getModule<Notifications>();

	//	if (notiMod->isEnabled())
		{
			g_Data.getClientInstance()->getGuiData()->displayClientMessageF("[%sAuraV4%s] %s%s %s%s%s", BLUE, WHITE, GRAY, enabled ? "Enabled" : "Disabled", BOLD, WHITE, this->getModuleName());
		}
		if (enabled)
			this->onEnable();
		else
