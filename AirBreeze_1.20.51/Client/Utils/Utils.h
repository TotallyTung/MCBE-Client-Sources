...
public:
	...
	static auto getResourcePath(void) -> std::string;
	static auto setDebugPath(std::string)->void;
public:
	static auto debugOutput(std::string, bool consoleMode = false)->void;
public:
	template<typename T>
...
public: //thank nrg for way to download file
	static bool doesPathExist(std::string path, bool dir);
	static void downloadFile(std::string url, std::string path);
