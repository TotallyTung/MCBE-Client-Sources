...
PacketMine::PacketMine() : Module("PacketMine", "NULL", Category::PLAYER) {
	slider<float>("Range", "NULL", ValueType::FLOAT_T, &range, 3.f, 10.f);
	boolsetting("Countinue", "Still Mines that blocks", &countinue);
	slider<float>("Countinue Speed", "NULL", ValueType::FLOAT_T, &Cspeed, 0.01f, 1.f);
	boolsetting("Silent Switch", "Auto switch to the best pickaxe in hotbar silent", &silenSwitch);
	boolsetting("Silent Back", "Switch back to old hotbar after silent switch", &silentBack);
	enumsetting("Break Mode", "NULL", { "Auto", "Keybind" }, &breakMode);
	keybindsetting("Break keybind", "NULL", &breakKeybind);
}
Vec3<float> possrot;
