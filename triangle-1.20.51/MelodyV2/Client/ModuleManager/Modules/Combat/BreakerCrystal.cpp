...
bool swing11;
bool packets1;
int packetmultiplier11 = 0;
CrystalBreaker1::CrystalBreaker1() : Module("CrystalBreaker", "Auto attack end crystals.", Category::COMBAT) {
	...
	slider<int>("Hit Attempts", "NULL", ValueType::INT_T, &hitattempts1, 0, 20);
	boolsetting("Swing", "NULL", &swing11);
	boolsetting("Packets", "NULL", &packets1);
	slider<int>("Packet Multiplier", "NULL", ValueType::INT_T, &packetmultiplier11, 0, 20);
}

int CrystalBreaker1::getBestWeaponSlot() {
...
Vec2<float> GetRotations1(Vec3<float> playerEyePos, Vec3<float> targetPos) {
...
void CrystalBreaker1::onNormalTick(Actor* actor) {
#include "../Exploit/AntiDesync.h"
#include "../../../Client.h"
	...
	if (!targetList.empty()) {
		...
		if (TargetDis < MaxSrange) {
			...
			rotAngle = GetRotations1(localPlayer->stateVectorComponent->pos, targetList[0]->stateVectorComponent->pos.sub(Vec3<float>(0.f, 0.9f, 0.f)));
			if (rotMode == 2) {
				...
				for (auto& target : targetList) {
					if (target->stateVectorComponent->pos.dist(localPlayer->stateVectorComponent->pos) < ARange) {
						if (!(hurttime && target->hurtTime > 0)) {
							...
							if (packets1) {
								for (auto i = 0; i < packetmultiplier11; i++) {
									mc.getClientInstance()->loopbackPacketSender->sendToServer(&inter);
								}
							}
							for (auto i = 0; i < hitattempts1; i++) {
								gm->attack(target);
							}
							if (swing11) {
								localPlayer->swing();
...
void CrystalBreaker1::onSendPacket(Packet* packet, bool& shouldCancel) {
...
void CrystalBreaker1::onImGuiRender(ImDrawList* d) {
