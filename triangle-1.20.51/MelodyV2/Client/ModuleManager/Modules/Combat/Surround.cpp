...
Surround::Surround() : Module("Surround", "Place blocks around you.", Category::COMBAT) {
	boolsetting("Center", "NULL", &center);
	boolsetting("DisableComplete", "NULL", &disableComplete);
	enumsetting("Switch", "Only works for Obsidian", { "None", "Full", "Silent" }, &switchMode);
	boolsetting("Render", "NULL", &render);
	colorsetting("Color", "NULL", &color);
	colorsetting("LineColor", "NULL", &lineColor);
	boolsetting("Above place", "Place blocks above your head", &Placeabove); 
    slider<int>("Delay", "NULL", ValueType::INT_T, &DELAY_TICKS, 2, 20);
}
            ...
            if (playerAABB.intersects(blockAABB)) {
                ...
                for (int i : {-1, 1}) {
                    for (int j : {-1, 1}) {
                        ...
                        renderPositions.push_back(sidePos); 
                    }
                }
                for (int i : {-1, 1}) {
                    for (int j : {-1, 1}) {
                        ...
                        if (playerAABB.intersects(cornerAABB)) {
                            ...
                            renderPositions.push_back(adjustedPos); 
                        }
                    }
                }
            }
