...
void ItemId::execute(const std::vector<std::string>& args) {
    ...
    if (args.size() < 2) {
        char errorMsg[] = "[%snuke.triangle%s] %sInvalid command! Usage: .(item) <hold/h>";
    ...
    case 'h':
        if (subCommand == "hold") {
            ...
            if (itemStack->isValid()) {
                ...
                char msg[256];
                sprintf(msg, "[%snuke.triangle%s] %s Item ID is -> : %d", DARK_PURPLE, WHITE, GREEN, itemId);
                mc.DisplayClientMessage(msg);
            }
            else {
                mc.DisplayClientMessage("[%snuke.triangle%s] %sYou are not holding a valid item!", DARK_PURPLE, WHITE, RED);
            }
        }
        else {
            mc.DisplayClientMessage("[%snuke.triangle%s] %sInvalid command! Usage: .(item) hold", DARK_PURPLE, WHITE, RED);
        }
        break; 
    default:
