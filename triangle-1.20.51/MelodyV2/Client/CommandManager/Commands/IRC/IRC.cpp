...
void IRC::execute(const std::vector<std::string>& args) {
    if (args.size() < 2) {
        char errorMsg[] = "[%snuke.triangle%s] %sInvalid command! Usage: .(IRC,internet,talk) words";
    ...
    case 'i':
        if (subCommand == "irc") {
            ...
            if (sendDiscordMessage(message)) {
                mc.DisplayClientMessage("[%snuke.triangle%s] %sMessage sent to Discord webhook successfully!", DARK_PURPLE, WHITE, GREEN);
            }
            else {
                mc.DisplayClientMessage("[%snuke.triangle%s] %sFailed to send message to Discord webhook.", DARK_PURPLE, WHITE, RED);
            }
        }
        break;
    }
