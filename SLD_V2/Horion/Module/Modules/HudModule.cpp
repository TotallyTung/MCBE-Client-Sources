...
HudModule::HudModule() : IModule(0, Category::HUD, "Displays ArrayList/TabGUI") {
	...
	registerBoolSetting("Welcomer", &welcomer, welcomer);
	registerFloatSetting("WelcomerX", &this->welcomerX, this->welcomerX, 0.f, g_Data.getClientInstance()->getGuiData()->windowSize.x);
	registerFloatSetting("WelcomerY", &this->welcomerY, this->welcomerY, 0.f, g_Data.getClientInstance()->getGuiData()->windowSize.y);
	registerIntSetting("Welcomer Red", &this->customr, this->customr, 0, 255);
	registerIntSetting("Welcomer Green", &this->customg, this->customg, 0, 255);
	registerIntSetting("Welcomer Blue", &this->customb, this->customb, 0, 255);
...
void HudModule::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	{ // Welcomer by hydroflame521

		if (!(g_Data.getLocalPlayer() == nullptr || !welcomer)) {
			std::string myName = g_Data.getLocalPlayer()->getNameTag()->getText();
			std::string welcomeMessage = "Welcome to SLD " + myName + " :^";
			vec2_t welcomePos = vec2_t(welcomerX, welcomerY);
			DrawUtils::drawText(welcomePos, &welcomeMessage, MC_Color(customr, customg, customb), scale);
		}
