...
namespace MCR {
	...
	std::vector<Vec3<float>> getBoxCorners3D(Vec3<float> lower, Vec3<float> upper);
	void rotateBoxCorners3D(std::vector<Vec3<float>>& vertices, Vec3<float> rotationPoint, Vec3<float> angles);
	void translateBox3D(std::vector<Vec3<float>>& vertices, Vec3<float> shift);
	void drawRawBox3D(std::vector<Vec3<float>> vertices, bool outline = false, bool onUi= true);
	...
	void drawRectangle(const Vec4<float>& rect, const UIColor& color);
	...
	void drawLine(const Vec2<float>& p1, const Vec2<float>& p2, const UIColor& color);
