...
void MCR::onRenderScreen(MinecraftUIRenderContext* ctx) {
	...
	MCR::drawText(Vec2<float>(1.5f, 1.f), "Boost", ModuleManager::getModule<Colors>()->getColor(), 0.5f);
	MCR::drawText(Vec2<float>(1.5f + MCR::getTextWidth("Boost ", 0.5f), 1.f), "v2", UIColor(255, 255, 255), 0.5f);
...
std::vector<Vec3<float>> MCR::getBoxCorners3D(Vec3<float> lower, Vec3<float> upper) {
	Vec3 diff = upper;
...
void MCR::rotateBoxCorners3D(std::vector<Vec3<float>>& vertices, Vec3<float> rotationPoint, Vec3<float> angles) {
...
void MCR::translateBox3D(std::vector<Vec3<float>>& vertices, Vec3<float> shift) {
	for (int i = 0; i < vertices.size(); i++) vertices[i].add(shift);
}
void MCR::drawRawBox3D(std::vector<Vec3<float>> vertices, bool outline, bool onUi) {
	if (vertices.size() != 8) return;

	static const uint8_t v[48] = {
		5, 7, 6, 4, 4, 6, 7, 5,
		1, 3, 2, 0, 0, 2, 3, 1,
		4, 5, 1, 0, 0, 1, 5, 4,
		6, 7, 3, 2, 2, 3, 7, 6,
	...
	UIColor fillColor = onUi ? UIColor(0, 255, 0, 60) : UIColor(255, 255, 255, 60);
	UIColor lineColor = onUi ? UIColor(0, 255, 0, 255) : UIColor(255, 255, 255, 255);

	for (int i = 0; i < 48; i++) {
		quad3dRenderList.push_back(Render3dData(vertices[v[i]], fillColor));
	}

	if (outline) {
		auto drawLine = [&](const Vec3<float>& a, const Vec3<float>& b) {
			line3dRenderList.push_back(Render3dData(a, lineColor));
			line3dRenderList.push_back(Render3dData(b, lineColor));
			};
...
void MCR::drawRectangle(const Vec4<float>& rect, const UIColor& color) {
	float x = rect.x;
	float y = rect.y;
	float width = rect.z;
	float height = rect.w;

	float thickness = std::max(1.0f, std::min(width, height) * 0.01f);

	drawQuad(
		Vec2<float>(x, y),
		Vec2<float>(x + width, y),
		Vec2<float>(x + width, y + thickness),
		Vec2<float>(x, y + thickness),
		color
	);

	drawQuad(
		Vec2<float>(x, y + height - thickness),
		Vec2<float>(x + width, y + height - thickness),
		Vec2<float>(x + width, y + height),
		Vec2<float>(x, y + height),
		color
	);

	drawQuad(
		Vec2<float>(x, y + thickness),
		Vec2<float>(x + thickness, y + thickness),
		Vec2<float>(x + thickness, y + height - thickness),
		Vec2<float>(x, y + height - thickness),
		color
	);

	drawQuad(
		Vec2<float>(x + width - thickness, y + thickness),
		Vec2<float>(x + width, y + thickness),
		Vec2<float>(x + width, y + height - thickness),
		Vec2<float>(x + width - thickness, y + height - thickness),
		color
	);
}
