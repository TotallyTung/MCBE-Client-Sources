...
bool TargetUtil::isTargetValid(Actor* target, bool isMob, float rangeCheck) {
	...
	if (target->getNameTag()->find("SeaHorse") != std::string::npos)
		return false;
	if (target->getNameTag()->find("XAlisamipriv") != std::string::npos)
		return false;
...
bool TargetUtil::isFriendValid(Actor* target, bool isMob, float rangeCheck) {
	...
	if (target == localPlayer)
		return false;
	if (ModuleManager::getModule<MidClick>()->isFriend(*target->getNameTag()))
		return false;

	if (target->getNameTag()->find("SeaHorse") != std::string::npos)
		return false;
	if (target->getNameTag()->find("Zep Ownss U") != std::string::npos)
		return false;
	if (ModuleManager::getModule<AntiBot>()->isEnabled()) {
		if (target->getNameTag()->length() <= 1 || target->getNameTag()->find("game selector") != std::string::npos) return false;
		if (target->aabbShape->width >= 0.70f && target->aabbShape->width <= 0.80f && target->aabbShape->height >= 2.0f && target->aabbShape->height <= 2.2f) return false;
