...
float DamageUtils::getExplosionDamage(const Vec3<float>& position, Actor* actor, float extrapolation, int ignoredBlockId) {
	...
	float exposure = Game.getClientInstance()->getRegion()->getSeenPercent(position, actor->getAABB());
	if (ignoredBlockId != 0) exposure = Game.getClientInstance()->getRegion()->getBlock(position.floor().toInt())->blockLegacy->blockId == ignoredBlockId ? 1.f : Game.getClientInstance()->getRegion()->getSeenPercent(position, actor->getAABB());
	else exposure = Game.getClientInstance()->getRegion()->getSeenPercent(position, actor->getAABB());
