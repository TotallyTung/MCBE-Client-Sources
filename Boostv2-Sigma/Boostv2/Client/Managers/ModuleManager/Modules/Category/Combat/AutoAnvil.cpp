...
AutoAnvil::AutoAnvil() : Module("AutoAnvil", "Places anvils on enemy heads", Category::COMBAT) {
    registerSetting(new BoolSetting("mobs", "Place on mobs", &mobs, false));
    registerSetting(new BoolSetting("swap", "swap", &swa3p1, true));
    registerSetting(new BoolSetting("silent swap", "swap", &s1lents3p, false));
    registerSetting(new SliderSetting<int>("y-offset", "Vertical offset", &yoffsetJ, 3, 0, 6));
    registerSetting(new SliderSetting<int>("delay", "Tick delay between placements", &delay, 5, 0, 20));
}
    ...
    for (Actor* actor : level->getRuntimeActorList()) {
        ...
        for (int attempt = 0; attempt < maxAttempts && !foundValidPosition; attempt++) {
            ...
            if (swa3p1) {
                ...
                Game.getLocalPlayer()->getgameMode()->buildBlock(blockPos, face, false);
                if (s1lents3p) {
