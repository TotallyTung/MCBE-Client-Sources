...
Arraylist::Arraylist() : Module("Arraylist", "Display module enabled", Category::RENDER) {
    ...
    registerSetting(new SliderSetting<int>("Offset", "NULL", &offset, 0, 0, 90));
}
...
bool Arraylist::sortByLength(Module* lhs, Module* rhs) {
    ...
    // Garantir que a keybind ?válida antes de adicionar
...
void Arraylist::onD2DRender() {
    ...
    for (auto& mod : moduleList) {
        ...
        if (mod->arraylistAnim > 0.f) {
            ...
            if (outline) {
                // Ajuste para esconder as linhas horizontais (não desenh?las)
                ...
                if (Enum2 == 1) {
                    ...
                    if (index == 0) {
                        // Se voc?não quiser desenhar as linhas horizontais, pode comentar ou remover as próximas linhas
