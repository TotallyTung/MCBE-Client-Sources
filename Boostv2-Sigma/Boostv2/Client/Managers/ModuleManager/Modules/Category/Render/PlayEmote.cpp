...
PlayEmote::PlayEmote() : Module("AutoEmote", "Auto-doing Random Emote", Category::RENDER) {
	registerSetting(new SliderSetting<int>("Emote delay", "NULL",  &EmoteDelay, EmoteDelay, 0, 20));
}
...
void PlayEmote::onNormalTick(LocalPlayer* actor) {
	...
	if (Emotedelaytick >= EmoteDelay) {
		...
		localPlayer->playEmote(emoteUUIDs[currentEmoteIndex]);
		currentEmoteIndex++;
		Emotedelaytick = 0;
	}
	else {
