...
NoClip::NoClip() : Module("NoClip", "Fly with inside of blocks", Category::MOVEMENT) {
...
void NoClip::onNormalTick(LocalPlayer* localPlayer) {
    ...
    if (Game.canUseMoveKeys()) {
        ...
        localPlayer->stateVector->pos.y += Glide;
        // Atualizando a posi��o do jogador diretamente
        Vec3<float> newPos;
        ...
        localPlayer->setPos(newPos);  // Atualiza a posi��o com setPos
    }
}
...
void NoClip::onSendPacket(Packet* packet) {
    ...
    if ((packet->getName() == "PlayerAuthInputPacket" || packet->getName() == "MovePlayerPacket")) {
        ...
        // Modificaci�n para que el movimiento de subida y bajada sea de 0.02 y -0.02 cada 3 ticks
        if (paip) {
            if (bypass) {
