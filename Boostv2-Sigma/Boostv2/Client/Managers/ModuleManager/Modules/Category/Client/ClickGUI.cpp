...
inline float fremate() {
	// Adjust this based on the actual FPS or frame rate calculation method
	return 60.f;  // Example fixed frame rate, replace with your frame rate calculation method
}
...
void ClickGUI::Render() {
	...
	D2D::drawNamePng(screenSize.x, screenSize.y, screenSize.x, screenSize.y);
	int steps = 100; 
	...
	for (int i = 0; i < circleList.size(); i++) {
		...
		p->startPos.y -= p->speed * (60.f / fremate());
			...
			for (auto& mod : window->moduleList) {
				...
				if (&mod == &window->moduleList.back()) {
					...
					D2D::fillRectangle(Vec4<float>(
						mRectPos.x,
						topY,
						mRectPos.x + borderWidth,
						bottomY
					), mainColor);

					D2D::fillRectangle(Vec4<float>(
						mRectPos.z - borderWidth,
						topY,
						mRectPos.z,
						bottomY
					), mainColor);
					...
					if (mod->extended) {
							...
							case SettingType::SLIDER_S: {
								...
								if (sliderSettingBase->valueType == ValueType::INT_T) {
									...
									if (draggingSliderSettingPtr == sliderSettingBase) {
										float draggingPercent = std::clamp((mousePos.x - sRectPos.x) / (sRectPos.z - sRectPos.x), 0.f, 1.f);
										value = (int)(minValue + std::round((maxValue - minValue) * draggingPercent));
									}
								...
								// Progress bar draw
								float valuePercent = std::clamp((animatedValue - minValue) / (maxValue - minValue), 0.f, 1.f);
						...
						Vec4<float> sLineRectRight = Vec4<float>(
							mRectPos.z - sLinePaddingX - sLineWidth, 
							startY,
							mRectPos.z - sLinePaddingX,
							endY
						);

						D2D::fillRectangle(sLineRectRight, mainColor);
					...
					if (&mod == &window->moduleList.back()) {
						...
						Vec4<float> finalBottomLine = Vec4<float>(
							mRectPos.x,
							yOffset,
							mRectPos.z,
							yOffset + bottomLineHeight
						);
						D2D::fillRectangle(finalBottomLine, mainColor);
					}
