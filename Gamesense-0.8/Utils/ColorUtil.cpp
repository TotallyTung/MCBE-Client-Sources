...
MC_Color ColorUtil::rainbowColor(float seconds, float saturation, float brightness, long index)
{
	...
	float r, g, b = 0;
	Utils::HSVtoRGB(hue6, saturation, interface->brightness, r, g, b);
...
MC_Color ColorUtil::astolfoRainbow(int yOffset, int yTotal)
{
	...
	hue += 0.5f;
	float r, g, b = 0;
	Utils::HSVtoRGB(hue, 0.5, interface->brightness, r, g, b);
...
MC_Color ColorUtil::interfaceColor(int index)
{
	...
	{
	case 1:
		color = ColorUtil::rainbowColor(8, i->saturation, i->brightness, -index * 4.f);
		break; /* Rainbow */
	case 2:
		color = ColorUtil::astolfoRainbow(index / 5, 1000);
		break; /* Astolfo */
	case 0:
		color = ColorUtil::waveColor(i->r, i->g, i->b, i->r2, i->g2, i->b2, index * 3.f);
		break; /* Wave */
	case 3:
		color = ColorUtil::RGBWave(i->r, i->g, i->b, i->r2, i->g2, i->b2, index * 3.f);
		break; /* RGB Wave */
	case 4:
		color = ColorUtil::rainbowColor(8, 0.50f, i->brightness, -index * 4.f);
		break; /* Horion */
	}
...
MC_Color ColorUtil::rainbowColortwo(float seconds, float saturation, float brightness, long index)
{
	...
	return MC_Color(r * 255, g * 255, b * 255, 255.f);
}
...
MC_Color ColorUtil::RGBWavetwo(int red, int green, int blue, int red2, int green2, int blue2, long index)
{
	...
	int bluePart = (int)(b * blue * inverse_percent + b * blue2 * aids123);
	auto arrayl = moduleMgr->getModule<ArrayList>();
	return MC_Color(redPart, greenPart, bluePart, 255.f);
}
...
MC_Color ColorUtil::interfaceColortwo(int index)
{
	...
	auto color = ColorUtil::rainbowColor(8, 1.F, i->brightness, 255.f);

	switch (i->color.getSelectedValue())
	{
	case 0:
		color = ColorUtil::rainbowColortwo(8, i->saturation, i->brightness, -index * 4.f);
		break; /* Rainbow */
	case 1:
		color = ColorUtil::astolfoRainbowtwo(index / 5, 1000);
		break; /* Astolfo */
	case 2:
		color = ColorUtil::waveColortwo(i->r, i->g, i->b, i->r2, i->g2, i->b2, index * 3.f);
		break; /* Wave */
	case 3:
		color = ColorUtil::RGBWavetwo(i->r, i->g, i->b, i->r2, i->g2, i->b2, index * 3.f);
		break; /* RGB Wave */
	case 4:
		color = ColorUtil::rainbowColortwo(8, 0.50, i->brightness, -index * 4.f);
		break; /* Horion */
	}
	return MC_Color(color);
}
