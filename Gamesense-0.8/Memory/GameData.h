...
#include "../Gamesense/Config/AccountInformation.h"
...
struct NotificationBox {
	...
	//NotificationBox(std::string title2, std::string message2, float duration2, float maxDuration2) : title(title2), message(message2), duration(duration2), maxDuration(maxDuration2) {};
};
...
private:
	...
	std::vector<std::shared_ptr<NotificationBox>> AllNotificationBox;
...
public:
	...
	std::vector<std::shared_ptr<NotificationBox>>& getAllNotification() {
		return this->AllNotificationBox;
	}

	void addNotification(std::string title, std::string message, float duration) {
		//auto box = std::make_shared<NotificationBox>(title, message, duration, duration);
		auto box = std::make_shared<NotificationBox>();
		box->title = title;
		box->message = message;
		box->duration = duration;
		box->maxDuration = duration;

		this->AllNotificationBox.push_back(box);
	}
