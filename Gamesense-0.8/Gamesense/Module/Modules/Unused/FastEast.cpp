...
FastEat::FastEat() : IModule(0, Category::PLAYER, "Eat food almost instant") {
	...
	Mode.addEntry("Duration", 0);
	Mode.addEntry("Timer", 1);
...
const char* FastEat::getModuleName() {
	if (Mode.getSelectedValue() == 0) name = std::string("FastEat ") + std::string(GRAY) + std::string("Duration");
	else name = std::string("FastEat ") + std::string(GRAY) + std::string("Timer");
	return name.c_str();
}
