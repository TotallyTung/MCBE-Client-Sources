...
EnumEntry::EnumEntry(const string _name, const unsigned char value, const char* tooltip2)
{
	name = _name;
	val = value;
}

EnumEntry::EnumEntry(const char *_name, const unsigned char value, const char* tooltip2)
{
	name = _name;
	val = value;
}
...
SettingEnum &SettingEnum::addEntry(EnumEntry entr, const char* tooltip2)
{
...
SettingEnum &SettingEnum::addEntry(const char *name, int value, const char* tooltip2)
{
	auto etr = EnumEntry(name, value, tooltip2);
	bool SameVal = false;
	for (auto it : Entrys)
		SameVal |= it.GetValue() == etr.GetValue();
...
IModule::IModule(int key, Category c, const char *tooltip)
{
	...
	this->registerKeybindSetting(std::string("Key"), &this->keybind, this->keybind);
	this->registerBoolSetting(string("enabled"), &this->enabled, false);
	this->registerBoolSetting(string("Hide"), &this->shouldHide, false);
	this->ModulePos = vec2_t(0.f, 0.f);
}

void IModule::registerFloatSetting(std::string name, float *floatPtr, float defaultValue, float minValue, float maxValue, float step, const char* tooltip2)
{
#ifdef DEBUG
	...
	strcpy_s(setting->tooltip2, 255, tooltip2); // Tooltip

	settings.push_back(setting); // Add to list
}

SettingEntry *IModule::registerIntSetting(std::string name, int *intPtr, int defaultValue, int minValue, int maxValue, const char* tooltip2)
{
#ifdef DEBUG
	if (minValue > maxValue)
		__debugbreak(); // Minimum value is bigger than maximum value
...
void IModule::registerEnumSetting(std::string name, SettingEnum *ptr, int defaultValue, const char* tooltip2)
{
	SettingEntry *setting = new SettingEntry();
...
SettingEntry *IModule::registerEnumSettingGroup(std::string name, SettingEnum *ptr, int defaultValue, const char* tooltip2)
{
	SettingEntry *setting = new SettingEntry();
...
void IModule::registerBoolSetting(std::string name, bool *boolPtr, bool defaultValue, const char* tooltip2)
{
	SettingEntry *setting = new SettingEntry();
...
void IModule::onImGuiRender(ImDrawList* d)
{
}
...
void IModule::setEnabled(bool enabled)
{
	if (this->enabled != enabled)
	{
		...
		if (player == nullptr ||
			...
			isHoldMode() || strcmp(screenName.c_str(), "start_screen") == 0)
			shouldShow = false;

		if (notifications->isEnabled()) {
			std::string state = enabled ? "Enabled" : "Disabled";
			
			std::string text = state + " " + this->getRawModuleName();
			g_Data.addNotification(text.c_str(), "", 2.f);
		}
...
const char* IModule::getTooltip2()
{
