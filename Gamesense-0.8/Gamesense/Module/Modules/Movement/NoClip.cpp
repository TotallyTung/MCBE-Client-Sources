...
NoClip::NoClip() : IModule(0x0, Category::MOVEMENT, "NoClip through walls on all axis")
{
	...
	this->registerBoolSetting("NoPacket", &this->NoPacket, this->NoPacket);
}
...
const char *NoClip::getRawModuleName()
{
	return ("NoClip");
}
...
{
	auto ncGlide = moduleMgr->getModule<NoClipGlide>();
	float oldGlide = 0.00;
	gm->player->aabb.upper.y = gm->player->aabb.lower.y - (float)1.8f;
	gm->player->velocity = vec3_t(0, 0, 0);
	auto glideModEffective = ncGlide->glide;
	C_GameSettingsInput *input = g_Data.getClientInstance()->getGameSettingsInput();

	if (g_Data.canUseMoveKeys() && !ncGlide->isEnabled())
	{
	...
	if (ncGlide->isEnabled()) {
		gm->player->velocity.y = glideModEffective;
	}

	if (!ncGlide->isEnabled()) {
		gm->player->velocity.y = oldGlide;
	}
}
