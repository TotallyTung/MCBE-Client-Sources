...
void CommandMgr::initCommands() {
	...
	commandList.push_back(new XUIDCommand());
	commandList.push_back(new FriendListCommand());
	commandList.push_back(new ChatSuffixCommand());
...
void CommandMgr::execute(char* message) {
	if (message != nullptr) {
		...
		g_Data.getClientInstance()->getGuiData()->displayClientMessageF("[%sGame%ssense%s] %sCommand '%s' could not be found!", WHITE, GREEN, WHITE, RED, cmd.c_str());
	}
}
