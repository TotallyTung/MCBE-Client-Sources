...
bool SpammerCommand::execute(std::vector<std::string>* args) {
	...
	if (option == "text") {
		...
		clientMessageF("[Gamesense] %sSpammer message set to %s%s%s!", GREEN, GRAY, text.c_str(), GREEN);
		return true;
	} else if (option == "delay") {
		int delay = assertInt(args->at(2));
		if (delay < 1) {
			clientMessageF("[Gamesense] %sDelay needs to be 1 or more!", RED);
			return true;
		} else {
		...
		spammer->bypass = state;
		clientMessageF("[Gamesense] %sBypass set to %s%s%s!", GREEN, GRAY, state ? "true" : "false", GREEN);
		return true;
		...
		for (int i = 0; i < times; i++) {
			...
			textPacket.message.setText(text + (spammer->bypass ? (" | " + Utils::randomString(8)) : ""));
