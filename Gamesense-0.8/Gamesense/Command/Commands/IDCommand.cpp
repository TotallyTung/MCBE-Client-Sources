	...
	if (args->at(1) == "pointing") {
		...
		if (pointing->hasEntity()) {
			...
			if (entity != nullptr) {
				...
				clientMessageF("[Gamesense] Entity Name: %s", entityName.c_str());
				clientMessageF("[Gamesense] Entity ID: %s", entityID.c_str());
			}
		} else {
			clientMessageF("[Gamesense] Block Name: %s", blockName);
			clientMessageF("[Gamesense] Block ID: %d", blockID);
		}
		return true;
		...
		if (stack->item != nullptr) {
			...
			g_Data.getGuiData()->displayClientMessageF("[Gamesense] Item Name: %s", name);
			g_Data.getGuiData()->displayClientMessageF("[Gamesense] Item ID: %lld", id);
		}
		return true;
	}
	return false;
