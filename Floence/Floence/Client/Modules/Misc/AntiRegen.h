...
public:
    AntiRegen(int keybind = Keys::NUM_0, bool enabled = false) :
        Module("AntiRegen", "Misc", "Cover up all redstones around you cuz of the fat nigga redstone eaters", keybind, enabled)
    {
            ...
            for (int x = -range; x < range; x++) {
                for (int y = -range; y < range; y++) {
                    for (int z = -range; z < range; z++) {
                        ...
                        if (block->GetBlockLegacy()->getTileName() == "tile.redstone_ore" || block->GetBlockLegacy()->getTileName() == "tile.lit_redstone_ore")
                        {
