...
public:
    ...
    {
        addEnum("Mode", "The mode for the delay", { "Milliseconds", "Destroy Speed", "Hive"}, &DelayMode);
        ...
        // Milliseconds
        addSlider("Time MS", "The delay for breaking in millisecounds", &destroyMs, 1, 1000, 1);
        addSlider("Other MS", "The delay for breaking blocks that sorround the redstone in millisecounds", &blockDestroyMs, 1, 1000, 1);
...
class KBTests : public Module
{
public:
    KBTests(int keybind = 7, bool enabled = true) :
        Module("KBTests", "Misc", "Funny fly for damage!", keybind, enabled)
    {
        addEnum("Style", "Da mode of KBTests!", { "JumpOnce" }, &Style);
        addBool("Halt", "Reset playerspeed when u toggle offed this", &Halt);
        addSlider("Timer", "Fly Timer gays", &FlyTimer, 1.f, 30.f);
    }
    ...
    void onEvent(ActorBaseTickEvent* event) override {
        ...
        if (Halt) {
            if (onGround) {
                ...
                Address::getTimerClass()->setMainTimerSpeed(FlyTimer);
