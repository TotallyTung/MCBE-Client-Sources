...
public:
	PingHolder(int keybind = Keys::NUM_0, bool enabled = false) :
		Module("PingHolder", "Misc", "Ping holder", keybind, enabled)
	{
		addSlider("Ping", "The ping", &Global::PingHolderValue, 0, 1000);
		addBool("Randomize", "randomize the ping between Low and High", &Global::RandomizePing);
		addSlider("Low Ping", "lowest ping", &Global::LowPingValue, 0.f, 1000);
		addSlider("High Ping", "highest ping", &Global::HighPingValue, 0.f, 1000);
	}
        ...
        if (Global::PingHolderValue) {
            if (rkc != nullptr && Address::getLocalPlayer() != nullptr) {
                ...
                if (!jIp.empty()) {
                    ...
                    bool c4 = TimeUtils::hasTimeElapsed("DelayForRelease", 100, true);
