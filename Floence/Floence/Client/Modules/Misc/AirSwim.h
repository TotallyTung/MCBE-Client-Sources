...
class Javascaf : public Module
{
public:
    Javascaf(int keybind = 82, bool enabled = false) :
        Module("Javascaf", "Misc", "its basically scaffold,but java tower!", keybind, enabled)
    {
        addEnum("Rotations", "The rotation mode", { "Normal", "Moonwalk", "None" }, &rotations);
        ...
        addEnum("TowerMode", "The tower mode", { "Jump", "Velocity", "CustomVelo" }, &towerMode);
        ...
        addBool("Andromeda", "Makes the scaffold place above and below you", &Andromeda);
        addSlider("PlacesPerTick", "how many blocks it can place in tick", &placesPerTick, 01, 10);
        addSlider("Distance", "The place distance (how far to place blocks)", &range, 0.1, 8);
        addSlider("Extend", "Distance your position to target block", &extend, 0, 10);
        addSlider("TowertTimer", "Speed of timer(tower)", &TowerTimer, 0.1f, 30);
        addSlider("TowertVelo", "Speed of velocity(tower)", &TowerVelocity, 0.1f, 30);
    }
...
private:
    ...
    float extend = 0;
    float range = 5.f;
    float TowerTimer = 15.f;
    float TowerVelocity = 0.5f;
...
public:
    ...
    bool canPlace(Vector3<float> pos) {
        ...
        return Address::getBlockSource()->getBlock(pos.floor().ToInt())->GetTileName() == "tile.air";
    }
    ...
    Vector2<float> getNoneAngle(const Vector2<float>& angle) {
        float x = angle.x;
        float y = angle.y;
        while (x > 0.f)
            x -= 0.f;
    ...
    bool buildBlock(Vector3<float> blockBelow) {
        ...
        if (Address::getBlockSource()->getBlock(Vector3<int>(blockBelow.ToInt()))->GetBlockLegacy()->getTileName() == "tile.air") {
            ...
            for (auto current : checklist) {
                ...
                if (Address::getBlockSource()->getBlock(calc)->GetBlockLegacy()->getTileName() != "tile.air") {
    ...
    bool predictBlock(Vector3<float> blockBelow) {
        ...
        for (const Vector3<int>& blockOffset : checkBlocks) {
            ...
            if (Address::getBlockSource()->getBlock(Vector3<int>(currentBlock))->GetBlockLegacy()->getTileName() == "tile.air") {
    ...
    void onEvent(ActorBaseTickEvent* event) override {
        ...
        if (towerMode == 2) {
            //            float speedOfTower = 0.35;
            bool space = Global::Keymap[VK_SPACE];
            static bool oldSpace = false;
            if (space) {
                //                player->getStateVector()->Velocity.y = speedOfTower;
                player->getStateVector()->Velocity.y = (TowerVelocity);
                Address::getTimerClass()->setMainTimerSpeed(TowerTimer);
            }
            else if (oldSpace) {
...
//            float speedOfTower = 0.35;
            ...
            if (space) {
//                player->getStateVector()->Velocity.y = speedOfTower;
                player->getStateVector()->Velocity.y = 0.5f;
                Address::getTimerClass()->setMainTimerSpeed(15.f);
            }
            else if (oldSpace) {
                Address::getTimerClass()->setMainTimerSpeed(20.f);
//                player->getStateVector()->Velocity.y = -5.f;
...
//                player->getStateVector()->Velocity.y = -5.f;
    ...
    void renderAeolusBlockAmountDisplay() {
        if (foundBlock) {
            ...
            string text = to_string(totalCount) + " blocks left";
            //            ImRenderUtil::drawText(Vector2<float>(pos2.x + 10, pos2.y + 46), &version, ColorUtils::getClientColor(1.9, 1, 1, 80), 1.1, 1, true);

            float textSize = 1.25;

            //            int ind = 0;
            //            float charWidth = ImRenderUtil::getTextWidth(&text, 2.3);
            //            float charHeight = ImRenderUtil::getTextHeight(2.3);
            //            int colorIndex = ind * 80;
            //            UIColor color = Vector2<float>(pos2.x + 10, pos2.y + 46),ColorUtils::getClientColor(1.9, 1, 1, 400);
            UIColor color = ColorUtils::getClientColor(1.9, 1, 1, 1);
            ...
            //ImRenderUtil::fillShadowRectangle(rectPos, ColorUtils::getClientColor(1.9, 1, 1, 400), 0.4f, 60, 0);

            ImRenderUtil::drawText(textPos, &text, UIColor(255, 255, 255), textSize, 1, true);
    ...
    void onEvent(PacketEvent* event) {
        ...
        Vector2<float> angleNone = player->getStateVector()->Position.CalcAngle(player->getStateVector()->Position, BlockBelow.ToFloat());
        ...
        case 1: //Moonwalk
            ...
            break;
        case 2: //NONE
            angleNone = getNoneAngle(angleNone);

            if (event->Packet->getId() == PacketID::PlayerAuthInput) {
                auto* pkt = reinterpret_cast<PlayerAuthInputPacket*>(event->Packet);
                if (pkt) {
                    pkt->rotation.x = angleNone.x;
                    pkt->rotation.y = angleNone.y;
                    pkt->headYaw = angleNone.y;
                }
            }
            break;
    ...
    void onEvent(IntersectsTickEvent* event) override {
        ...
        case 1: //Moonwalk
            ...
            break;
        case 2: //None
            angleNone = getNormAngle(angleNone);
            player->setPitch(angleNone.x);
            player->setHeadYaw(angleNone.y);
            player->setBodyRotation(angleNone.y);
            break;
