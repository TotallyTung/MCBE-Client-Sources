...
public:
    ...
    // Get the disabled state of the module.
    ...
    void addSlider(const std::string name, const std::string description, float* value, float min, float max, float round = 0.1) {
        settings.emplace_back(new Setting(name, description, value, min, max, round));
    }
    ...
    void setRenderSetting(std::string name, bool render) {
    ...
    virtual void onEvent(class AttackEvent* event) {}; // On ActorBaseTick event.
    ...
    virtual void onEvent(class IntersectsTickEvent* event) {}; // On IntersectsTick event.
