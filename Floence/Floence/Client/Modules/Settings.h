...
public:
    ...
    Setting(const std::string& name, const std::string& description, float* value, float min, float max, float round)
        : name(name), description(description), type(SettingType::SLIDER), value(value), Min(min), Max(max), Round(round) {
    }
