...
public:
	InventoryManager(int keybind = Keys::NONE, bool enabled = false) :
		Module("InventoryCleaner", "Player", "Clean ur inventory!", keybind, enabled)
	{
		addEnum("Mode", "When to manage", { "InvOnly" }, &mode);
		...
		addBool("KeepBows", "Do not drop bows", &keepBows);
...
public:
	...
	void onEvent(ContainerTickEvent* event) override
	{
		...
		auto playerinventory = player->getSupplies()->inventory;
		...
		if (!allItems.empty() && autoArmor) {
			for (int i = 0; i < 4; i++) {
				...
				if (betterArmorSlot != -1 && allItems.size() != 36) {
					if (armorcontainer->getItem(i)->item != nullptr) {
						controller->handlePlaceAll(Containers::Armor, i);
					}
					if (isHotbar) {
						controller->handlePlaceAll(Containers::Hotbar, betterArmorSlot);
					}
					else {
						controller->handlePlaceAll(Containers::Inventory, betterArmorSlot);
					}
					return;
				}
			}
		...
		if (!allItems.empty() && sortSword) {
			ItemStack* stack = playerinventory->getItem(0);
			...
			if (betterSwordSlot != -1 && allItems.size() != 36) {
				if (playerinventory->getItem(0)->item != nullptr) {
					controller->handlePlaceAll(Containers::Hotbar, 0);
				}
				if (isHotbar) {
					controller->handlePlaceAll(Containers::Hotbar, betterSwordSlot);
				}
				else {
					controller->handlePlaceAll(Containers::Inventory, betterSwordSlot);
				}
				return;
			}
		}
			...
			if (toolsMode == 1) { //Hotbar
				for (auto itemStatus : allItems) {
					if (!itemStatus.ishotbar && (itemStatus.itemstack->getItem()->isPickaxe() || itemStatus.itemstack->getItem()->isAxe())) {
						controller->handlePlaceAll(Containers::Inventory, itemStatus.containerslot);
					}
				}
			}
				...
				for (auto itemStatus : allItems) {
					if (itemStatus.ishotbar && (itemStatus.itemstack->getItem()->isPickaxe() || itemStatus.itemstack->getItem()->isAxe())) {
						controller->handlePlaceAll(Containers::Hotbar, itemStatus.containerslot);
					}
				}
			}
		}
	...
	bool isUselessItem(ItemStack* stack) {
		...
		//Bows
		if (stack->getItem()->nameContains("bow") || stack->getItem()->nameContains("arrow")) {
			if (keepBows) {
				return false;
			}
		}
