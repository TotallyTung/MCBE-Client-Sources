...
Aimbot::Aimbot() : IModule(0, Category::COMBAT, "Automatically aims at the nearest entity.") {
	addSlider("Range", &range, range, 3.f, 8.f);
	addBool("Require Click", &click, click);
	addBool("Only Swords/Axes", &sword, sword);
	addBool("Vertical", &vertical, vertical);
	addSlider("Horizontal Speed", &horizontalspeed, horizontalspeed, 10.f, 90.f);
	addSlider("Vertical Speed", &verticalspeed, verticalspeed, 10.f, 90.f);
	addSlider("Horizontal Range", &horizontalrange, horizontalrange, 20.f, 180.f);
	addSlider("Vertical Range", &verticalrange, verticalrange, 20.f, 180.f);
	addBool("Aimlock", &lock, lock);
}
