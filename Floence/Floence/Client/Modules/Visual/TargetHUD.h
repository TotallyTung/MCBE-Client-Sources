...
public:
	...
	{
		addEnum("Style", "The style of the targethud", { "Zephyr" }, &Style);
...
public:
	...
	void onEvent(ImGuiRenderEvent* event) {
		...
		if (getModuleByName("aura")->isDisabled())  return; 
		...
		float inScale = shouldDisplay ? inEase.easeOutExpo() : inEase.easeOutBack();
		...
		static Vector4<float> rect2 = Vector4<float>(pos.x, pos.y, pos.x + 250, pos.y + 80);
		...
		if (!targetList.empty()) {
			...
			if (track3d) {
				...
				rect2 = Vector4<float>(pos.x, pos.y, pos.x + (250 * textSize), pos.y + (3 * textSize));
				rect = Vector4<float>(pos.x, pos.y, pos.x + (250 * textSize), pos.y + (50 * textSize));
				textSize = 1.f;
			}
		...
		ImRenderUtil::fillRectangle(rect, UIColor(0, 0, 0), 0.6f, 0.f);//0.6 14
		ImRenderUtil::fillRectangle(rect2, ColorUtils::getClientColor(1.9, 1, 1, 180), 1.f, 0.f);//0.6 14
		...
		}		ImRenderUtil::drawText(Vector2<float>(pos.x + 10, pos.y + 5), &targetName, UIColor(255, 255, 255), 1.2f * textSize, 1.f, true); //x75 y23
		ImGui::PopFont();
		...
		ImRenderUtil::drawText(Vector2<float>(pos.x + 10, pos.y + 25), &targetHealthStr, UIColor(255, 255, 255), 1.f * textSize, 1.f, true);
		ImGui::PopFont();
		...
		ImColor HurtColor = IM_COL32_WHITE;

		static ParticleEngine particleMgr;

		if (damageTime >= 1) {
			UIColor UIHeadColor = UIColor(255, 114, 118); //255, 114, 118
			HurtColor = ImColor(UIHeadColor.r, UIHeadColor.g, UIHeadColor.b, UIHeadColor.a);
			ImRenderUtil::fillShadowRectangle(rect, UIHeadColor, 1 * (DamageAnimation / 10), 40.f * (DamageAnimation / 10), 0, 14.f);
		}
