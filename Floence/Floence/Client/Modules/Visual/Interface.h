...
public:
    ...
    {
        addEnum("Mode", "The font for the display", { "Minecraft", "ProductSans", "AhhEmojis" }, &Global::interfont);
        addEnum("Color", "The theme color", { "GreenBlue", "Astolfo", "BubbleGum", "Rainbow", "Tenacity", "Wave", "Cherry", "Xextreame", "White", "BlackRed", "test", "test2", "Light Blue", "Vanilla" }, &Global::ClientColor);
    ...
    void onEvent(ImGuiRenderEvent* event) override {
        ...
        if (keystrokes) {
            DrawKeyStroke(1, 1, "W", player->getMoveInputHandler()->mInputState.mUp);
            DrawKeyStroke(0, 0, "A", player->getMoveInputHandler()->mInputState.mLeft);
            DrawKeyStroke(1, 0, "S", player->getMoveInputHandler()->mInputState.mDown);
            DrawKeyStroke(2, 0, "D", player->getMoveInputHandler()->mInputState.mRight);
        }
    ...
    void DrawKeyStroke(int xIndex, int yIndex, std::string key, bool held) {
        ...
        ImRenderUtil::drawGradientText(textPos, &name, scale, 1, true,2.5 ,0.75);
