...
public:
    ...
    {
        addEnum("Style", "The style of the Level Info", { "Vanilla", "RGB" }, &style);
    ...
    void onEvent(ImGuiRenderEvent* event) override {
        ...
        oss3.precision(2);
        oss3 << std::fixed << "BPS: " << player->getHorizontalSpeed();
        ...
        if (showBPS) {
            drawLevelInfo(Utils::combine(oss3.str().c_str()), index++);
    ...
    void drawLevelInfo(std::string tileDescr, int index)
    {
        if (style == 0) {
            ...
            ImRenderUtil::drawText(tilePos, &tileDescr, ColorUtils::getClientColor(1.9, 1, 1, 80), 1.18f, 0.9f, true);
