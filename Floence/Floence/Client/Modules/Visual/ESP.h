        ...
        if (selfrender && !Global::stopESP) {
            ...
            if (style == 1) {
                ...
                if (player->getHurtTime() >= 1) {
                    ImRenderUtil::drawCorners(position.add(Vector3<float>(-0.43, -1.7, -0.43)), position.add(Vector3<float>(0.43, 0.4, 0.43)), UIColor(255, 255, 255).lerp(UIColor(255, 255, 255), ImRenderUtil::getDeltaTime() * 10.f), (float)fmax((thickness / 10), 0.7 / (float)fmax(1, Address::getLocalPlayer()->getRenderPosition().distance(player->getRenderPosition()))));
                }
                else {
