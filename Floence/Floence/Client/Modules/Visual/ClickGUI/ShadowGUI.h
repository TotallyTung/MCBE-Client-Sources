...
public:
    ...
    void render(float animation, float inScale, int& scrollDirection, char* searchQuery, char* h, bool blur) {
        ...
        if (Global::interfont == 2) {
            ...
            ImRenderUtil::fillRectangle(Vector4<float>(0, 0, ImRenderUtil::getScreenSize().x, ImRenderUtil::getScreenSize().y), UIColor(0, 0, 0), animation * 0.7f);
        ...
        std::string SearchQueryStr = strlen(searchQuery) == 0 ? " " : searchQueryLower; // note : raw text is Press any key to search modules...         but this cant gui move,alr removed
        ...
        if (!catPositions.empty()) {
            for (size_t i = 0; i < categories.size(); i++) {
                ...
                Vector4<float> catWindow = Vector4<float>(catPositions[i].x, catPositions[i].y, catPositions[i].x + catWidth, catPositions[i].y + moduleY + catWindowHeight)
                    .scaleToPoint(Vector4<float>(ImRenderUtil::getScreenSize().x / 2, ImRenderUtil::getScreenSize().y / 2, ImRenderUtil::getScreenSize().x / 2, ImRenderUtil::getScreenSize().y / 2), inScale);
                //UIColor rgb = ColorUtils::getClientColor(2.5, 1 * 1, 1, i * 7);
                UIColor rgb = ColorUtils::getAstolfoRainbow(2.5, 1 * 1, 1, moduleY * 0.7);
                ...
                for (const auto& mod : modsInCategory) {
                    ...
                    //UIColor rgb = ColorUtils::getClientColor(2.5, 1 * 1, 1, moduleY * 0.7);
                    UIColor rgb = ColorUtils::getAstolfoRainbow(2.5, 1 * 1, 1, moduleY * 0.7);
                    ...
                    if (mod->getCategory() == categories[i]) {
                            ...
                            for (const auto& setting : mod->getSettings()) {
                                ...
                                case SettingType::BOOL: {
                                    ...
                                    if (rect.y > catRect.y + 0.5f) {
                                        ...
                                        //Vector4<float> BallRect = *(bool*)setting->getValue() ? Vector4<float>(smoothScaledRect.z - 17, smoothScaledRect.y + 1, smoothScaledRect.z - 1, smoothScaledRect.w - 4) : Vector4<float>(smoothScaledRect.x + 17, smoothScaledRect.y + 1, smoothScaledRect.x + 1, smoothScaledRect.w - 4);
                                        ...
                                        ImRenderUtil::fillCircle(circleRect, 5, *(bool*)setting->getValue() ? rgb : UIColor(0, 0, 0), 1, 5.f);
                                        //ImRenderUtil::fillShadowCircle(circleRect, 5, *(bool*)setting->getValue() ? rgb : UIColor(15, 15, 15), 1, 40, 0); aeaeaeaeaeaeaeaeaeaeaeae
                                        ...
                                        ImRenderUtil::drawText(Vector2(rect.x + 5.f, cSetRectCentreY), &setName, UIColor(220, 220, 220), textSize, animation, true); //Bool Text Color
                                    }
                                    break;
                                }
                                case SettingType::ENUM: {
                                    ...
                                    if (setting->enumSlide > 0.001)
                                    {
                                        for (int j = 0; j < numValues; j++)
                                        {
                                            ...
                                            if (rect2.y > catRect.y + 0.5f)
                                            {
                                                ...
                                                ImRenderUtil::drawText(Vector2<float>(rect2.x + 5.f, cSetRectCentreY), &enumValue, UIColor(220, 220, 220), textSize, animation, true); //Enum/Style select no nakano yatu
                                            }
                                        }
                                    }
                                    ...
                                    if (rect.y > catRect.y + 0.5f) {
                                        ...
                                        ImRenderUtil::drawText(Vector2<float>(rect.x + 5.f, cSetRectCentreY), &settingName, UIColor(220, 220, 220), textSize, animation, true); //Enum/Style no name/title
                                        ImRenderUtil::drawText(Vector2<float>((rect.z - 5.f) - ValueLen, cSetRectCentreY), &settingString, UIColor(220, 220, 220), textSize, animation, true); //Mode
                                    }
                                ...
                                case SettingType::SLIDER: {
                                    ...
                                    if (rect.y > catRect.y + 0.5f)
                                    {
                                        ...
                                        //ImRenderUtil::fillShadowRectangle(Vector4<float>(rect.x, (catPositions[i].y + catHeight + moduleY + modHeight) - 3, rect.x + setting->sliderEase, rect.w), rgb, animation, 50.f, 0); aeaeaeaeaeaeaeaeaeaeaeae
                                        //ImRenderUtil::fillRectangleCustom(Vector4<float>(newbarlol.z - 6.f, newbarlol.y - 0, newbarlol.z + 6, newbarlol.w + 0.5), UIColor(255, 255, 255), animation, Vector4<float>(20, 20, 20, 20));
                                        //ImRenderUtil::fillCircle(setting->sliderEase, 5, UIColor(255, 255, 255), 1, 5.f);
                        ...
                        if (modRect.y > catRect.y + 0.5f)
                        {
                            ...
                            ImRenderUtil::drawText(modPosLerped, &modName, UIColor(mod->isEnabled() ? UIColor(255, 255, 255) : UIColor(255, 255, 255)).lerp(UIColor(255, 255, 255), mod->cAnim), textSize, animation, true);
                ...
                if (ClickGUIManager::isBinding) {
                    tooltip = Utils::combine("Binding ", lastMod->getName() );
                ...
                ImRenderUtil::fillShadowRectangle(catRect, UIColor(0, 0, 0), animation, 100.f, 200.f, ImDrawFlags_ShadowCutOutShapeBackground);
                ImRenderUtil::fillRectangle(catRect, UIColor(40, 40, 40), animation);
                ...
                // Calculate the centre of the rect
                float cRectCentreX = catRect.x + ((catRect.z - catRect.x) - ImRenderUtil::getTextWidth(&catName, textSize * 1)) / 2;
                ...
                // Draw the string
                ImRenderUtil::drawText(Vector2(catRect.x + 10, cRectCentreY), &catName, UIColor(255, 255, 255), textSize * 1.14, animation, false);
