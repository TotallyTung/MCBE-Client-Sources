...
public:
 ...
 //           ImRenderUtil::fillRectangle(Vector4<float>(0, 0, ImRenderUtil::getScreenSize().x, ImRenderUtil::getScreenSize().y), rgb, animation * 0.38f);
        ...
        if (!catPositions.empty()) {
            for (size_t i = 0; i < categories.size(); i++) {
                ...
                Vector4<float> catWindow = Vector4<float>(catPositions[i].x, catPositions[i].y, catPositions[i].x + catWidth, catPositions[i].y + moduleY + catWindowHeight)
                    .scaleToPoint(Vector4<float>(ImRenderUtil::getScreenSize().x / 2, ImRenderUtil::getScreenSize().y / 2, ImRenderUtil::getScreenSize().x / 2, ImRenderUtil::getScreenSize().y / 2), inScale);
                //UIColor rgb = ColorUtils::getAstolfoRainbow(2.5, 1 * 0.5, 1, moduleY * 2);
                UIColor rgb = ColorUtils::getClientColor(2.5, 1, 1 * 0.5, (moduleY * 2));
                //UIColor rgb = ColorUtils::getClientColor(2.5, 1 * 0.5, 1, moduleY * 2.5);
                //UIColor rgb = ColorUtils::RainbowDark(2.5, (moduleY * 2)), ColorUtils::RainbowDark(2.5, (moduleY * 2) + 400), 1, 1);
                ...
                for (const auto& mod : modsInCategory) {
                    ...
                    if (mod->getCategory() == categories[i]) {
                            ...
                            for (const auto& setting : mod->getSettings()) {
                                ...
                                case SettingType::BOOL: {
                                    ...
                                    if (rect.y > catRect.y + 0.5f) {
                                        ...
                                        ImRenderUtil::fillRectangle(rect, UIColor(40, 40, 40), animation);
                                        ...
                                        ImRenderUtil::fillCircle(circleRect, 5, *(bool*)setting->getValue() ? rgb : UIColor(0, 0, 0), 1, 12.f);
                                ...
                                case SettingType::ENUM: {
                                    ...
                                    if (setting->enumSlide > 0.001)
                                    {
                                        for (int j = 0; j < numValues; j++)
                                        {
                                            ...
                                            if (rect2.y > catRect.y + 0.5f)
                                            {
                                                ...
                                                ImRenderUtil::fillRectangle(rect2, UIColor(40, 40, 40), animation);
                                    ...
                                    if (rect.y > catRect.y - modHeight) {
                                        ImRenderUtil::fillGradientOpaqueRectangle(Vector4<float>(rect.x, rect.w, rect.z, rect.w + 10.f * setting->enumSlide * animation), UIColor(40, 40, 40), UIColor(40, 40, 40), 0.F * animation, 0.55F * animation);
                                        //fewahfehwuiahfewhiuygfeawihoigyutvfyguyihuojiuhibugvyfcvgubhinjonhiugvyfcryvgubhinjomnihbugvyfctfyvgubhinjomkjnihbugvyfctdfyvgubhinjoihbugvyf
                                    }
                                    break;
                                }
                                case SettingType::SLIDER: {
                        ...
                        if (modRect.y > catRect.y + 0.5f)
                        {
                            ...
                            ImRenderUtil::drawText(textPos, &text, UIColor(255, 255, 255), 1.3f * inScale, inScale, true);
                        ...
                        if (modRect.y > catRect.y - modHeight) {
                            ...
                            ImRenderUtil::fillGradientOpaqueRectangle(Vector4<float>(modRect.x, modRect.w, modRect.z, modRect.w + 10.f * mod->cAnim * animation), UIColor(40, 40, 40), UIColor(40, 40, 40), 0.F * animation, 0.55F * animation);
                        }
                        moduleY += modHeight;
                    }
                }
                ...
                ImRenderUtil::fillRectangle(catRect, UIColor(0, 0, 0), animation); //UIColor(0, 0, 0)
                ...
                ImRenderUtil::drawText(Vector2(catRect.x + 10, cRectCentreY), &Symbol, UIColor(255, 255, 255), textSize * 1.f, animation, true);
                ImGui::PopFont();

                // Draw the string
                ImRenderUtil::drawText(Vector2(catRect.x + 34, cRectCentreY), &catName, UIColor(255, 255, 255), textSize * 1.14, animation, true);
...
#pragma endregion
            ...
            if (!tooltip.empty()) {
                ...
                ImRenderUtil::fillRectangle(tooltipRect, UIColor(40, 40, 40), animation, 0.f);
