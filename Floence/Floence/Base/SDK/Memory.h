...
namespace Memory {
    ...
    bool HookFunction(void* pTarget, T pDetour, void* pOriginal, const char* SignatureIdName) {
        ...
        if (!initialized)
        {
            ...
            {
                FileUtils::debugOutput(xorstr_("[Floence] failed to initialize MinHook for the client"));
                return false;
            }
        }
        ...
        {
            FileUtils::debugOutput(Utils::combine(xorstr_("[Floence] failed to create hook for "), SignatureIdName).c_str());
            return false;
        }

        if (MH_EnableHook(pTarget) != MH_OK)
        {
            FileUtils::debugOutput(Utils::combine(xorstr_("[Floence] failed to enable hook for "), SignatureIdName).c_str());
            return false;
        }

        FileUtils::debugOutput(Utils::combine(xorstr_("[Floence] successfully applied hook for "), SignatureIdName).c_str());
