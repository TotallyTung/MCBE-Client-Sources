...
public:
    ...
    float getDestroyRate(Block const& block) {
        ...
        if (!Sig) {
            Sig = Memory::findSig(xorstr_("E8 ? ? ? ? 0F 28 F8 49 8b 4E ? E8"));
            uintptr_t out = (uintptr_t)Sig;
            out += 1 + sizeof(float) + *reinterpret_cast<int*>(out + 1);
            Sig = (void*)out;
        }
        static getDestroyRate getDestroyRateFunc = reinterpret_cast<getDestroyRate>(Sig);
