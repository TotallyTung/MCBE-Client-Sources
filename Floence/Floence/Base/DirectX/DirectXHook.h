...
void callRender() { // onImGui CallBack for modules
	...
	//LoginInfo - Made by キリン
	GuiData* guidata = Address::getClientInstance()->getGuiData();
	Vector2<float> screenSize = guidata->mcResolution;
	float textSize = 2.8f; // ここテキストサイズ //2.1
	//string text = "Tozic is gay";
	string text = " ";

	float textWidth = ImRenderUtil::getTextWidth(&text, textSize);
	float textHeight = ImRenderUtil::getTextHeight(textSize);
	float offset = -16.f; // 距離

	float renderx = screenSize.x - textWidth - offset + 115.f;
	float rendery = screenSize.y - textHeight - offset;
	Vector2<float> textPos = Vector2<float>(renderx, rendery);
	ImGui::PushFont(ImGui::GetIO().Fonts->Fonts[4]);
	ImRenderUtil::drawGradientText(textPos, &text, /*UIColor(255, 255, 255),*/ 1.4f, 1.4f, true, 2.5, 120);
}
...
public:
	bool Initialize() override {
		...
		if (kiero::init(kiero::RenderType::D3D12) != kiero::Status::Success && kiero::init(kiero::RenderType::D3D11) != kiero::Status::Success) {
			FileUtils::debugOutput("[Floence] [DirectX] failed to initialize hook");
			return false;
		}
		...
		if (kiero::bind(index, (void**)&oResize, resizeBuffersCallback) != kiero::Status::Success) {
			FileUtils::debugOutput("[Floence] [DirectX] failed to create hook for resize buffer");
			return false;
		}
		...
		if (kiero::bind(index2, (void**)&oPresent, D3D12_PresentDetour) != kiero::Status::Success) {
			FileUtils::debugOutput("[Floence] [DirectX] failed to create hook for present");
			return false;
		}
