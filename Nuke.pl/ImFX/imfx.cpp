...
bool ImFX::NewFrame(class ::ID3D11Device* d3d11Device, IDXGISurface* dxgiBackBuffer, float fxdpi)
{
    if (d2dInit == false && SUCCEEDED(D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, &d2dFactory))) {
        ...
        d2dDeviceContext->CreateEffect(CLSID_D2D1Turbulence, &turbulenceEffect);
        if (turbulenceEffect == nullptr) {
            return false;
        }
...
struct ColorCallbackData {
    ...
    ColorCallbackData(ImVec4 color, std::optional<ImVec4> rounding, std::optional<ImVec4> clipRect = std::optional<ImVec4>()) {
        this->color = color;
        this->rounding = rounding;
        this->clipRect = clipRect;
    }
...
void ColorCallback(const ImDrawList* parent_list, const ImDrawCmd* cmd) {
    ...
    ID2D1PathGeometry* roundedRectGeo = CreateRoundRect(clipRect.x, clipRect.y, clipRect.z - clipRect.x, clipRect.w - clipRect.y, rounding.x, rounding.y, rounding.z, rounding.w);

    ID2D1SolidColorBrush* colorBrush = nullptr;
    d2dDeviceContext->CreateSolidColorBrush(D2D1::ColorF(data->color.x, data->color.y, data->color.z, data->color.w), &colorBrush);
...
struct ShadowCallbackData {
    ...
    ShadowCallbackData(float strength, float rounding, ImVec4 color, std::optional<ImVec4> clipRect = std::optional<ImVec4>()) {
        this->strength = strength;
        this->rounding = rounding;
        this->color = color;
        this->clipRect = clipRect;
...
void ShadowCallback(const ImDrawList* parent_list, const ImDrawCmd* cmd) {
    ...
    if (shadowBitmap == nullptr)
    {
        ...
        D2D1_BITMAP_PROPERTIES1 newLayerProps = D2D1::BitmapProperties1(D2D1_BITMAP_OPTIONS_TARGET, sourceBitmap->GetPixelFormat());
...
bool ImFX::AddShadow(float strength, ImVec4 color, std::optional<ImVec4> clipRect)
{
    if (!d2dInit) {
        return false;
    }
    ...
    float rounding = ImGui::GetStyle().FrameRounding;
    ShadowCallbackData* data = new ShadowCallbackData(strength, rounding, color, clipRect);
    currentDrawlist.back()->AddCallback(ShadowCallback, data);
    return true;
}

struct DisplacementMapCallbackData {

};

void DisplacementMapCallback(const ImDrawList* parent_list, const ImDrawCmd* cmd) {
    if (!d2dInit) {
        return;
    }
    ...
    DisplacementMapCallbackData* data = (DisplacementMapCallbackData*)cmd->UserCallbackData;
