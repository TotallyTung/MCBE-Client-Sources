...
CrystalAura::CrystalAura() : IModule(0, Category::COMBAT, "Destroys nearby End Crystals.") {
	...
	if (ptr != nullptr)
		if (ptr->getEntity() == nullptr && ptr->rayHitType == 0)
			if (Game.getLocalPlayer()->region->getBlock(ptr->block)->toLegacy()->blockId == 49 ||
				...
				DrawUtils::drawBox(ptr->block.toVec3t().add(0.f, 1.5f, 0.f),ptr->block.add(1).toVec3t().add(0.f, 1.5f, 0.f), .3f);
			}
}
