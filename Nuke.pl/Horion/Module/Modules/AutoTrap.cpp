...
void findEntityat(Entity* currentEntityat, bool isRegularEntity) {
	static auto AutoTrapMod = moduleMgr->getModule<AutoTrap>();

	if (currentEntityat == nullptr)
		return;

	if (currentEntityat == Game.getLocalPlayer())  // Skip Local player
		return;

	if (!Game.getLocalPlayer()->canAttack(currentEntityat, false))
		return;

	if (!Game.getLocalPlayer()->isAlive())
		return;

	if (!currentEntityat->isAlive())
		return;


	if (!Target::isValidTarget(currentEntityat))
		return;

	float dist = (*currentEntityat->getPos()).dist(*Game.getLocalPlayer()->getPos());
	if (dist < AutoTrapMod->range) {
		targetListat.push_back(currentEntityat);
