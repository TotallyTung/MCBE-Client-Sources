...
bool GiveCommand::execute(std::vector<std::string> *args) {
	...
	//clientMessageF(u8"%sDEBUG:%s Will give %d stacks!", RED, GREEN, stackCount);
	//clientMessageF(u8"%sDEBUG:%s Will give %d as a remainder!", RED, GREEN, count);

	// Give us all the stacks of the items we want.
	for (unsigned int i = 0; i < stackCount; i++) {
		//clientMessageF(u8"%sDEBUG:%s Giving stack %d of items!", RED, GREEN, i + 1);
		std::string tag;
		bool success = false;
	...
	if (count >= 1) {
		//clientMessageF(u8"%sDEBUG:%s Giving remaining %d items!", RED, GREEN, count);
		std::string tag;
		bool success = false;
	...
	if (args->size() > 4) {
		...
		clientMessageF(u8"%s%s Successfully loaded mojangson !", Utils::getCommandClientName(), GREEN);
	}
	if (args->size() > 4) {
	...
	clientMessageF(u8"%s%sSuccessfully gave items!", Utils::getCommandClientName(), GREEN);
	return true;
}
...
bool GiveCommand::giveItem(uint8_t count, int itemId, uint8_t itemData, std::string &tag) {
	...
	if (cStack == nullptr || *cStack == nullptr || **cStack == nullptr) {
		clientMessageF(u8"%s%sInvalid item ID!", Utils::getCommandClientName(), RED);
		return false;
	}
...
bool GiveCommand::giveItem(uint8_t count, TextHolder &text, uint8_t itemData, std::string &tag) {
	...
	if (*cStack == nullptr) {
		clientMessageF(u8"%s%sInvalid item name!", Utils::getCommandClientName(), RED);
		return false;
	}
	itemStack = new ItemStack(***cStack, count, itemData);
