...
static constexpr float textPaddingX = 1.6f;  // 2.f
static constexpr float textPaddingY = 1.f;   // 1.f
static constexpr float textSize = 1.f;
static constexpr float textHeight = textSize * 9.0f;
static constexpr float categoryMargin = 1.f;
static constexpr float paddingRight = 22.f;
static constexpr float crossSize = textHeight / 4.f;
static constexpr float crossWidth = 0.25f;
static constexpr float backgroundAlpha = 0.6;
...
void ClickGui::renderCategory(Category category) {
	...
	if (ourWindow->moduleList.empty()) {
		getModuleListByCategory(category, &ourWindow->moduleList);
	}

	const std::vector<std::shared_ptr<IModule>>& moduleList = ourWindow->moduleList;
	...
	if (ourWindow->isExtended || ourWindow->isInAnimation) {
		...
		const float cutoffHeight = 999999.f;  // roundf(Game.getGuiData()->heightGame * 0.75f) + 0.5f /*fix flickering related to rounding errors*/;
		int moduleIndex = 0;
		for (auto& mod : moduleList) {
			...
			int keybind = mod->getKeybind();
			if (keybind != 0x0) {
				textStr = textStr + " [" + std::string(Utils::getKeybindName(keybind)) + "]";
			}
