...
#include "../Library/Fonts/BahnSchrift.h"
#include "../Library/Fonts/Mojangles.h"
#include "../Library/Fonts/ProductSansRegular.h"
#include "../SDK/Tag.h"
...
Present oPresent;
bool BoolShowBackGroundValue = false;
...
void Hooks::Init() {
	...
	// Signatures
	{
		...
		g_Hooks.KeyMapHook = make_unique<FuncHook>(key, Hooks::KeyMapHookCallback);
...
//		"48 8D 3D ? ? ? ? 80 B8 ? ? ? ? ? 75 19 48 8B 88 ? ? ? ? 48 8B 11 4C 8B 42 28 8B 50 10";
		...
		g_Hooks.AppPlatform_getGameEditionHook = make_unique<FuncHook>(getGameEdition, Hooks::AppPlatform_getGameEdition);
		...
		g_Hooks.LevelRendererPlayer__renderNameTagsHook = make_unique<FuncHook>(renderNameTags, Hooks::LevelRendererPlayer__renderNameTags);
		...
		g_Hooks.ActorisInWallHook = make_unique<FuncHook>(actorisInWall, Hooks::Actor__isInWall);
		...
		//g_Hooks.testFunctionHook = make_unique<FuncHook>(testFunc, Hooks::testFunction);
	...
	// Vtables
	{
		...
		if (Game.getClientInstance()->loopbackPacketSender != nullptr) {
			...
			if (packetSenderVtable == nullptr)
				...
				g_Hooks.LoopbackPacketSender_sendToClientHook = make_unique<FuncHook>(packetSenderVtable[4], Hooks::LoopbackPacketSender_sendToClient); //I use the second sendToClient
			}
		}
		...
		// LocalPlayer::vtable
		{
			...
			if (localPlayerVtable == 0x0)
				...
				g_Hooks.Actor_startSwimmingHook = make_unique<FuncHook>(localPlayerVtable[201], Hooks::Actor_startSwimming);

				g_Hooks.Actor__setRotHook = make_unique<FuncHook>(localPlayerVtable[27], Hooks::Actor__setRot);

				g_Hooks.Actor_swingHook = make_unique<FuncHook>(localPlayerVtable[219], Hooks::Actor_swing);

				g_Hooks.JumpPowerHook = make_unique<FuncHook>(localPlayerVtable[345], Hooks::JumpPower); //jump from ground with movement proxy

				//g_Hooks.setPosHook = make_unique<FuncHook>(localPlayerVtable[19], Hooks::setPos);

				g_Hooks.Actor_baseTickHook = make_unique<FuncHook>(localPlayerVtable[49], Hooks::Actor_baseTick);
				...
				g_Hooks.Actor_aiStepHook = make_unique<FuncHook>(localPlayerVtable[291], Hooks::Actor_aiStep);

				g_Hooks.Actor_isOnFireHook = make_unique<FuncHook>(localPlayerVtable[102], Hooks::Actor_isOnFire);

				//g_Hooks.Actor__isInvisibleHook = make_unique<FuncHook>(localPlayerVtable[59], Hooks::Actor__isInvisible);
			}
		}
		...
		// GameMode::vtable
		{
			...
			if (gameModeVtable == 0x0)
				...
				g_Hooks.GameMode__stopDestroyBlockHook = make_unique<FuncHook>(gameModeVtable[4], Hooks::GameMode__stopDestroyBlock);

				g_Hooks.GameMode_buildHook = make_unique<FuncHook>(gameModeVtable[6], Hooks::GameMode_build);
...
void Hooks::onImGuiRender(ImDrawList* d) {
	//moduleMgr->getModule<ClientTheme>()->SaveEveryTime();
...
HRESULT Hooks::presentCallback(IDXGISwapChain3* pSwapChain, UINT syncInterval, UINT flags) {
	...
	if (SUCCEEDED(pSwapChain->GetDevice(IID_PPV_ARGS(&d3d11Device)))) {
		...
		if (!g_Hooks.initImGui) {
			...
			io.Fonts->AddFontFromMemoryTTF((void*)Mojangles, sizeof(Mojangles), 500.f); // 200.f
...
void Hooks::chatLogHookFunc(__int64 a1, TextHolder* msg, uint32_t a2) {
	...
	static auto AutoReplyMod = moduleMgr->getModule<AutoReply>();
	static auto ChatMod = moduleMgr->getModule<ChatSuffix>();
	static auto GreeterMod = moduleMgr->getModule<Greeter>();
	static auto ChatBotMod = moduleMgr->getModule<ChatBot>();
	static auto player = Game.getLocalPlayer();

	if (message.getText() != nullptr && player != nullptr) {
		string textStr = message.getText();
		string MyName = player->getNameTag()->getText();
		if (ChatBotMod->isEnabled()) {
			if (textStr.find(u8"!tpaccept") != string::npos) {
				ChatBotMod->cmd1 = true;
			}
			if (textStr.find(u8"!locate") != string::npos) {
				ChatBotMod->cmd2 = true;
			}
			if (textStr.find(u8"!kill") != string::npos) {
				ChatBotMod->cmd3 = true;
			}
			if (textStr.find(u8"!hub") != string::npos) {
				ChatBotMod->cmd4 = true;
			}
			if (textStr.find(u8"!fact") != string::npos) {
				ChatBotMod->cmd5 = true;
			}
			if (textStr.find(u8"!health") != string::npos) {
				ChatBotMod->cmd6 = true;
			}
			if (textStr.find(u8"!ask") != string::npos) {
				ChatBotMod->cmd7 = true;
			}
		}
		if (GreeterMod->isEnabled()) {
			if (textStr.find(u8"§8[§a+§8]") != string::npos || textStr.find(u8"decided to join us for some PvP.") != string::npos) {
				string PlayerName;
				if (textStr.find(u8"decided to join us for some PvP.") != string::npos) PlayerName = regex_replace(string(textStr).substr(3), (regex)" decided to join us for some PvP.", "");;
				if (textStr.find(u8"§8[§a+§8]") != string::npos) PlayerName = string(textStr).substr(16);
				if (PlayerName != MyName) {
					std::vector<std::vector<std::string>> onJoiningMessage;
					onJoiningMessage.push_back({
						u8"Hello " + PlayerName,
						u8"Good to see you again " + PlayerName,
						u8"Oh hey " + PlayerName + u8", I didn't know you were coming back, glad to see you again",
						PlayerName + u8" welcome to this server!",
						u8"Nice to see you " + PlayerName,
						u8"Sup " + PlayerName,
						u8"Ayyyy its " + PlayerName,
						u8"0_0 its " + PlayerName,
					});
					onJoiningMessage.push_back({
						u8"学校に行かない君が好きだ " + PlayerName,
						u8"佐藤、お前学校来いよ、、、" + PlayerName,
						u8"学校行かずにkit漁ってる気持ちはどぉぉぉ？？？ " + PlayerName,
						u8"弱いね～笑だからクラスにも馴染めないんじゃない？笑笑 " + PlayerName,
						u8"保健室とーこー、ひまわり学級笑笑 " + PlayerName
					});
					onJoiningMessage.push_back({
						u8"にゃんにゃん//にぼにゃん!かわいいにゃん!!",
						u8"「こんにちは」" + PlayerName  + u8"「あ、えっと、あ、ぁ...」"
					});
					int selectedValue = GreeterMod->mode.getSelectedValue();
					if (selectedValue >= 1) if (PlayerName.find(u8" ") != string::npos) PlayerName = u8"@" + '"' + PlayerName + '"';
					Utils::sendChat(onJoiningMessage[selectedValue][rand() % onJoiningMessage[selectedValue].size()]);
				};
			}
			else if (textStr.find(u8"§8[§c-§8]") != string::npos || textStr.find(u8"had enough PvP for now.") != string::npos) {
				string PlayerName;
				if (textStr.find(u8"had enough PvP for now.") != string::npos) PlayerName = regex_replace(string(textStr).substr(3), (regex)" had enough PvP for now.", "");
				if (textStr.find(u8"§8[§c-§8]") != string::npos) PlayerName = string(textStr).substr(16);
				if (PlayerName != MyName) {
					std::vector<std::vector<std::string>> onLeavingMessage;
					onLeavingMessage.push_back({
						u8"Bye bye " + PlayerName + u8" you'll be missed",
						u8"Ahhh RIP, " + PlayerName + u8" left",
						u8"Ayyyy " + PlayerName + u8" left, lets have a party",
						u8"Cya " + PlayerName,
						PlayerName + u8" left, SIKE, oops nvm",
						u8"I think " + PlayerName + u8" got kicked?",
						u8"Did " + PlayerName + u8" crash or log?",
					});
					onLeavingMessage.push_back({
						PlayerName + u8" 早退するの？大丈夫？笑笑笑 ",
						u8"クラスメイト「なんで保健室行くの？」" + PlayerName + u8"「え、えっと、た、体調悪くて、、、」 ",
						PlayerName + u8" いいなあ特別学級笑笑"
					});
					onLeavingMessage.push_back({
						PlayerName + u8" 君、お母さん泣いてるよ？",
						PlayerName + u8" お前Googleのタブ100個くらい開いてそう笑 :nerd:",
						PlayerName + u8" ママの子宮からやり直せ笑"
					});
					int selectedValue = GreeterMod->mode.getSelectedValue();
					if (selectedValue >= 1) if (PlayerName.find(u8" ") != string::npos) PlayerName = u8"@" + '"' + PlayerName + '"';
					Utils::sendChat(onLeavingMessage[selectedValue][rand() % onLeavingMessage[selectedValue].size()]);
				}
			}
		};
		if (AutoReplyMod->isEnabled()) {
			if (textStr.find(u8"§6Me§7]") != string::npos) {
				Utils::sendCommand(u8"r AutoReply->" + AutoReplyMod->Message);
			};
		};
	};
...
void Hooks::ClientInstanceScreenModel_sendChatMessage(void* _this, TextHolder* text) {
	...
	if (*message == cmdMgr->prefix) {
		...
		call_once(flag, [] {
			Game.getClientInstance()->getGuiData()->displayClientMessageF("%sYour %s prefix is: \"%s%c%s\"", RED, Utils::getClientName(), YELLOW, cmdMgr->prefix, RED);
...
void Hooks::KeyMapHookCallback(unsigned char key, bool isDown) {
	...
	ClickGui::onKeyPressed((int)key, isDown, shouldCancel);
...
__int64 Hooks::RenderText(ScreenView* screenView, MinecraftUIRenderContext* renderCtx) {
	...
	if (!hookedRenderCtx) {
		...
		g_Hooks.drawImagekHook = make_unique<FuncHook>(VTable[7], Hooks::drawImageCallback);
		g_Hooks.drawImagekHook->enableHook();
	...
	//			if (clickGuiModule->guiMode.getSelectedValue() != 1) {
					ClickGui::render();
//				};
...
__int64 Hooks::drawTextCallback(Font* font, const float* pos, TextHolder* text, const float* color, float alpha, unsigned int textAlignment, const TextMeasureData* textMeasureData, const uintptr_t* caretMeasureData) {
	static auto oFunc = g_Hooks.drawTextHook->GetFastcall<__int64, Font*, const float*, TextHolder*, const float*, float, unsigned int, const TextMeasureData*, const uintptr_t*>();
	return oFunc(font, pos, text, color, alpha, textAlignment, textMeasureData, caretMeasureData);
}
__int64 Hooks::drawImageCallback(MinecraftUIRenderContext* renderCtx, TexturePtr* texturePtr, Vec2* imagePos, Vec2* imageDimension, Vec2* UvPos, Vec2* UvSize) {
	...
	int index = 0;
	static auto colorsMod = moduleMgr->getModule<Colors>();
	int curIndex = -index * colorsMod->spacing;
	MC_Color mainColor = colorsMod->getColorV2(curIndex);
	MC_Color subColor = ColorUtil::astolfoRainbow(curIndex);
	if (strcmp(texturePtr->getFilePath().getText(), "textures/ui/title") == 0) {
		Vec2 wid = Game.getGuiData()->windowSize;
		static std::string version = moduleMgr->getModule<HudModule>()->modversion + " beta";
		Vec2 textPos = Vec2((wid.x - DrawUtils::getTextWidth(&Utils::getClientName(), 6.f)) / 2.f, imagePos->y);
		Vec2 textPos1 = Vec2((wid.x - DrawUtils::getTextWidth(&version, 1.1f) + DrawUtils::getTextWidth(&Utils::getClientName(), 6.f)) / 2.f, imagePos->y + 50);
		DrawUtils::drawText(textPos, &Utils::getClientName(), mainColor, 6.f, 1.f, true);
		DrawUtils::drawText(textPos1, &version, subColor, 1.1f, 1.f, true);
		return 0;
	}
	auto mod = moduleMgr->getModule<CustomHotbar>();
	if (mod != nullptr) {
		if (mod->hotbartype.getSelectedValue() == 1 && mod->isEnabled()) {
			if (string(texturePtr->getFilePath().getText()).find("textures/ui/hotbar_") != string::npos) {
				DrawUtils::fillRectangle(Vec4(imagePos->x, imagePos->y, imagePos->x + imageDimension->x, imagePos->y + imageDimension->y), MC_Color(0, 0, 0), 0.5f);
				return 0;
			}
		}
		if (strcmp(texturePtr->getFilePath().getText(), "textures/ui/selected_hotbar_slot") == 0 && mod->isEnabled()) {
			static float lerpedPos = imagePos->x;
			lerpedPos = smoothLerp(imagePos->x, lerpedPos, mod->speed);
			if (mod->smooth.getSelectedValue() == 1) imagePos->x = lerpedPos;
			if (mod->hotbartype.getSelectedValue() == 1) {
				DrawUtils::fillRectangle(Vec4(imagePos->x + 1, imagePos->y + 1, imagePos->x + imageDimension->x - 1, imagePos->y + imageDimension->y - 1), MC_Color(0, 0, 0), 0.3f);
				DrawUtils::fillRectangle(Vec4(imagePos->x + 1, imagePos->y + imageDimension->y - 2, imagePos->x + imageDimension->x - 1, imagePos->y + imageDimension->y - 1), ColorUtil::rainbowColor(0, 1.f, 1.f, 0), 1.f);
				return 0;
			}
		}
	}
	if (strcmp(texturePtr->getFilePath().getText(), ("textures/blocks/end_stone")) == 0) return 0;
	if (strcmp(texturePtr->getFilePath().getText(), ("textures/blocks/netherrack")) == 0) return 0;
	if (strcmp(texturePtr->getFilePath().getText(), ("textures/blocks/dirt")) == 0) return 0;
	return func(renderCtx, texturePtr, imagePos, imageDimension, UvPos, UvSize);
}
...
void Hooks::Actor_lerpMotion(Entity* _this, Vec3 motVec) {
	...
	static auto SpeedMod = moduleMgr->getModule<NukeSpeed>();
	...
	if (SpeedMod->isEnabled() && SpeedMod->DamageBoosts) {
		SpeedMod->attack = true;
		SpeedMod->nowtick = random(SpeedMod->boostmintick, SpeedMod->boostmaxtick);
 }
	oLerp(_this, motVec);
}
...
void Hooks::LoopbackPacketSender_sendToServer(LoopbackPacketSender* a, Packet* packet) {
	...
	if (autoSneakMod->isEnabled() && Game.getLocalPlayer() != nullptr && autoSneakMod->silentlol && packet->isInstanceOf<PlayerActionPacket>()) {
...
//	if (packet->isInstanceOf<C_TextPacket>()) return;
	oFunc(a, packet);
}
...
float Hooks::LevelRendererPlayer_getFov(__int64 _this, float a2, bool a3) {
	...
	if (!moduleMgr->getModule<MoreFov>()->isEnabled())
		return oGetFov(_this, a2, a3);
	return moduleMgr->getModule<MoreFov>()->fovs;
}
...
void Hooks::MultiLevelPlayer_tick(EntityList* _this) {
	...
	LocalPlayer* localPlayer = Game.getLocalPlayer();
	if (localPlayer != nullptr && localPlayer->getGameMode() != nullptr) {
		moduleMgr->onTick(localPlayer->getGameMode());
		ClickGui::onTick(localPlayer->getGameMode());
	}
	oTick(_this);
...
bool Hooks::GameMode_startDestroyBlock(GameMode* _this, Vec3i* a2, uint8_t face, bool* a4) {
	...
	if (moduleMgr->isInitialized()) {
		if (Game.getLocalPlayer() != nullptr) {
			...
			if (Game.getLocalPlayer()->region->getBlock(*a2)->blockLegacy->blockId != 7 && packetMineMod->isEnabled()) {
...
void Hooks::ClickFunc(__int64 a1, char mouseButton, char isDown, __int16 mouseX, __int16 mouseY, __int16 relativeMovementX, __int16 relativeMovementY, char a8) {
	...
	ImGuiUtils::onMouseClickUpdate(mouseButton, isDown);

    if (clickGuiModule->isEnabled()) {
        ClickGui::onMouseClickUpdate(mouseButton, isDown);
//		ClickGuiMod::onMouseClickUpdate(mouseButton, isDown);
		if (mouseButton == 4) {
			ClickGui::onWheelScroll(isDown > 0);
//			ClickGuiMod::onWheelScroll(isDown > 0);
		}
...
static vector<Entity*> targetList_Health;

void findEntityHealth(Entity* currentEntity, bool isRegularEntity) {
	if (currentEntity->getEntityTypeId() == 319 && currentEntity != Game.getLocalPlayer()) targetList_Health.push_back(currentEntity);
}
Entity* getEntityByName(vector<Entity*> entitylist, string str) {
	for (Entity* ent : entitylist) if (ent->getNameTag()->getText() == str) return ent;
...
void Hooks::LevelRendererPlayer__renderNameTags(__int64 a1, __int64 a2, TextHolder* a3, __int64 a4) {
	...
	targetList_Health.clear();
	if (Game.getLocalPlayer() != nullptr) {
		Game.forEachEntity(findEntityHealth);
		...
		if (found != string::npos) {
			...
			auau = auau.substr(found + 1, found + 1);
		}
		stringstream ss;
		ss << auau;
		ss >> Healthy;
		if (!targetList_Health.empty()) {
			Entity* i = getEntityByName(targetList_Health, text);
			if (Healthy == 0) Healthy = 20;
			if (Target::isValidTarget(i) && i != Game.getLocalPlayer()) {
				i->Health = Healthy;
			};
		};
	};
	if ((moduleMgr->getModule<NameTags>()->isEnabled() && !moduleMgr->getModule<NameTags>()->HideMinecraftNametags) || !moduleMgr->getModule<NameTags>()->isEnabled())
...
__int64 Hooks::GameMode_build(GameMode* _this, Vec3i* Pos, unsigned char face, bool unsignedBool) {
	static auto func = g_Hooks.GameMode_buildHook->GetFastcall<__int64, GameMode*, Vec3i*, unsigned char, bool>();
	if (!moduleMgr->isInitialized())
		return func(_this, Pos, face, unsignedBool);
	if (moduleMgr->getModule<PacketMine>()->isEnabled() && moduleMgr->getModule<PacketMine>()->FastCancel)
		moduleMgr->getModule<PacketMine>()->Reset(_this);
	return func(_this, Pos, face, unsignedBool);
}
...
float Hooks::getDestroySpeed(Player* _this, Block& block) {
	...
	return (instaBreakMod->isEnabled() && !instaBreakMod->wookmc) ? INFINITY : oFunc(_this, block);
}
