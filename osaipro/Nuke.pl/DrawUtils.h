...
public:
	...
	static void updateGameDeltaTime(ScreenView* screenView);
	...
	static float getFontHeight(float textSize = 1.f);
	...
	static void fillRectangleA(Vec4 pos, MC_Color col);
	static void drawCenteredString(Vec2 pos, std::string* textStr, float textSize, MC_Color color, bool hasShadow);
	static void drawImage(std::string filePath, Vec2& imagePos, Vec2& imageDimension, Vec2& idk, Vec2& idk2, bool system);
	...
	static void drawText(const Vec2& pos, std::string* text, const MC_Color& color, float textSize = 1.f, float alpha = 1.f, bool shadow = true);
	static void drawCenteredText(Vec2 pos, std::string text, float size, float textOpacity);
	...
	static void drawFilledBox2D(Vec3 pos, float height = 0.f);
	...
	static void drawItem(ItemStack* item, const Vec2& ItemPos, float scale = 1.f);
	static void drawBarItem(ItemStack* stack, const Vec2& ItemPos, float scale = 1.f);
	static void drawTextInWorld(std::string* textToSay, const Vec3& location, float tsize, Vec3i tColor, Vec3i bgColor, float opacity);
	static Vec3 getOrigin();
	static void onMouseClickUpdate(int key, bool isDown);
	static Vec2 worldToScreen(const Vec3& world);
	static void drawHat(std::vector<Vec3>, float, float, Vec3, bool);
	static void drawLineStrip3D(const std::vector<Vec3>& points, int mode);
};
