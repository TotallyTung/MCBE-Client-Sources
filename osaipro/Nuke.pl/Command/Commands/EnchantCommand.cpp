...
bool EnchantCommand::execute(std::vector<std::string>* args) {
	...
	if (args->at(1) != "all") {
		try {
			...
			clientMessageF(u8"%sexception while trying to get enchant string", Utils::getCommandClientName());
			enchantId = assertInt(args->at(1));
		}
	}
	...
	if (strcmp(args->at(1).c_str(), "all") == 0) {
		...
		clientMessageF(u8"%s%sEnchant successful!", Utils::getCommandClientName(), GREEN);
	} else {
		void* EnchantData = malloc(0x60);
		if (EnchantData != nullptr)
			memset(EnchantData, 0x0, 0x60);
		...
		if (addEnchant(EnchantData, enchantPair)) {  // Upper 4 bytes = level, lower 4 bytes = enchant type
			...
			clientMessageF(u8"%s%sEnchant failed, try using a lower enchant-level", Utils::getCommandClientName(), RED);
