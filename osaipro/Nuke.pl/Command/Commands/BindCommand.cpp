...
bool BindCommand::execute(std::vector<std::string>* args) {
	...
	if (!modOpt.has_value()) {
		clientMessageF(u8"%sCould not find module with name: %s",  RED, moduleName.c_str());
		return true;
	}
	...
	if (args->size() >= 3) {
		...
		if (key.size() > 1) {
			...
			if (key == "none") {
				mod->setKeybind(0x0);
				clientMessageF(u8"%sSuccessfully unbound %s!",  GREEN, mod->getModuleName());
				return true;
			}
			...
			for (int i = 0; i < 190; i++) {
				...
				if (strcmp(needle, haystackLowercase) == 0) {
					mod->setKeybind(i);
					clientMessageF(u8"%sThe keybind of %s is now '%s'", GREEN, mod->getModuleName(), haystack);
					delete[] haystackLowercase;
					return true;
				}
				delete[] haystackLowercase;
			}
			clientMessageF(u8"%sInvalid key!",  RED);
			return true;
		}
		...
		if (keyCode >= 0x30 && keyCode <= 0x5A) {
			...
			if (!modOpt.has_value()) {
				...
				clientMessageF(u8"%sThe Keybind of %s is now '%c'",  GREEN, mod->getModuleName(), keyCode);
			}
		} else {
			clientMessageF(u8"%sInvalid Key! Outside of ascii range: %X",  RED, keyCode);
		}
		return true;
	} else {
		clientMessageF(u8"%s%s is currently bound to %s",  RED, mod->getModuleName(), Utils::getKeybindName(mod->getKeybind()));
		return true;
	}
