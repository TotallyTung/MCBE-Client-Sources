...
bool PlayerTeleportCommand::execute(std::vector<std::string>* args) {
	...
	if (pos.iszero()) {
		clientMessageF(u8"%s%sCouldn't find player: %s!", Utils::getCommandClientName(), RED, nameOfPlayer.c_str());
		return true;
	}
	Game.getLocalPlayer()->setPos(pos);
	clientMessageF(u8"%s%sTeleported to %s", Utils::getCommandClientName(), GREEN, playerName.c_str());
	return true;
}
