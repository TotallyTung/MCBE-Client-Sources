...
void showAimedBlockInfo() {
	...
	auto entity = level->getEntity();
	if (block != nullptr && level != nullptr && entity == nullptr && block->blockLegacy->blockId != 0) {
...
bool TestCommand::execute(std::vector<std::string>* args) {
	...
	itemId();
	showAimedBlockInfo();
	Game.getGuiData()->displayClientMessageF("ALL EFFECT: %i", Game.getLocalPlayer()->getAllEffect());
	return true;
}
