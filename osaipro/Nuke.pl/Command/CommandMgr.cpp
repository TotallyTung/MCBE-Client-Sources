...
void CommandMgr::initCommands() {
	...
	commandList.push_back(new ShowChatFriendList());
	commandList.push_back(new CombatInfoCommand());
	...
	commandList.push_back(new AutoReplyCommand());
	...
	for (auto LOL : commandList) {
		Utils::logC("Added Commands " + Utils::changeConsoleColor(2) + std::string(LOL->getCommand()), "INFO");
	};
}
...
void CommandMgr::execute(char* message) {
	...
	for (auto& c : commandList) {
		...
		for (const auto& alias : *aliases) {
			if (alias == cmd) {
				try {
					if (!c->execute(&args)) {
						Game.getGuiData()->displayClientMessageF("%s%s%sUsage: %s%c%s %s", Utils::getCommandClientName(), RED, BOLD, RESET, cmdMgr->prefix, c->getCommand(), c->getUsage(cmd.c_str()));
					}
				} catch (...) {
					Game.getGuiData()->displayClientMessageF("%s%s%sUsage: %s%c%s %s", Utils::getCommandClientName(), RED, BOLD, RESET, cmdMgr->prefix, c->getCommand(), c->getUsage(cmd.c_str()));
				}
	...
	Game.getGuiData()->displayClientMessageF("%s%sCommand '%s' could not be found!", Utils::getCommandClientName(), RED, cmd.c_str());
	...
	for (auto it = this->commandList.begin(); it != this->commandList.end(); ++it) {
		...
		if (commandCopy.find(cmd) != std::string::npos)                                                 // if it has the name in it but not completed
			Game.getGuiData()->displayClientMessageF("%s%sDid you mean to type: %s?", Utils::getCommandClientName(), RED, commandCopy.c_str());  // Tell the user what they might have ment
	}
}
