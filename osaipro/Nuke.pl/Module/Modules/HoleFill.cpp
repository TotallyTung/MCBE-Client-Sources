...
HoleFill::HoleFill() : IModule(0x0, Category::COMBAT, "Locate a good place to holecamp ;)") {
	...
	registerBoolSetting("Robot", &this->onclick, this->onclick);
...
void findEntityhf(Entity* currentEntityhf, bool isRegularEntity) {
	static auto AutoTrapMod = moduleMgr->getModule<HoleFill>();
						...
						for (int z = (int)pos->z - range; z < pos->z + range; z++) {
							for (int y = (int)pos->y - range; y < pos->y + range; y++) {
								...
								if (block != nullptr && block2 != nullptr && block3 != nullptr && block4 != nullptr) {
									...
									float airdist1 = (Vec3(pos->x, pos->y - 1, pos->z - 1)).dist(air);
									float airdist2 = (Vec3(pos->x, pos->y - 1, pos->z + 1)).dist(air);
									float airdist3 = (Vec3(pos->x - 1, pos->y - 1, pos->z)).dist(air);
									float airdist4 = (Vec3(pos->x + 1, pos->y - 1, pos->z)).dist(air);
									if (bedrock) {
										if ((blockId2 == 7 && blockId3 == 7 && blockId4 == 7 && blockId5 == 7 && blockId6 == 7 && blockId == 0) && (aircheck == 0)) {
											slotab = 0;
											slotab = supplies->selectedHotbarSlot;
											if (airdist1 > 0.49999f || airdist2 > 0.49999f || airdist3 > 0.49999f || airdist4 > 0.49999f) {
												for (int n = 0; n < 36; n++) {
