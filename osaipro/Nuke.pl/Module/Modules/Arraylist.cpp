...
Arraylist::Arraylist() : IModule(0, Category::CLIENT, "Display your enabled modules on the HUD!") {
	registerBoolSetting("Show Keybind", &showKey, showKey);
	...
	registerFloatSetting("Scale", &textSize, textSize, 0.1f, 2.f);
	registerEnumSetting("ModeType", &mode, 0);
	mode
		.addEntry("White", 0)
		.addEntry("Gray", 1);
}
...
void Arraylist::onImGuiRender(ImDrawList* d) {
	...
	for (auto it = modContainerList.begin(); it != modContainerList.end(); ++it) {
		index++; int curIndex = -index * moduleMgr->getModule<Colors>()->spacing;
		...
		if (i == 0 && bottom) rectPos.w += textPadding * 2.f;
		if (duration > 0.01f) {
			UIColor mainColor = moduleMgr->getModule<Colors>()->getColor(index);
