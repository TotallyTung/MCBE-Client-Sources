...
void DrawUtils::fillRectangleA(Vec4 pos, MC_Color col) {
	DrawUtils::setColor(col.r, col.g, col.b, col.a);
	DrawUtils::drawQuad({pos.x, pos.w}, {pos.z, pos.w}, {pos.z, pos.y}, {pos.x, pos.y});
}
void DrawUtils::drawCenteredString(Vec2 pos, std::string* textStr, float textSize, MC_Color color, bool hasShadow) {
	DrawUtils::drawText(Vec2(pos.x - DrawUtils::getTextWidth(textStr, textSize) / 2.F, pos.y - mcFont->getLineHeight() / 2.F), textStr, color, textSize, 1, hasShadow);
}
...
void DrawUtils::drawHat(std::vector<Vec3> posList, float opacity, float height, Vec3 pos, bool rgb) {
	auto tess = DrawUtils::get3dTessellator();
	DrawUtils::tess__begin(tess, 3, (int)posList.size());

	DrawUtils::setColor(1, 1, 1, opacity);
	int num = 0;
	for (const auto& p : posList) {
		MC_Color c = rainbowColor(0, 1.f, 1.f, num * 361 / 10);
		if (rgb) tess->color(c.r, c.g, c.b, opacity);
		auto pD = p.sub(origin);
		tess_vertex(tess, pD.x, pD.y, pD.z);
		tess_vertex(tess, pos.sub(origin).x, pD.y + height, pos.sub(origin).z);
		tess_vertex(tess, pD.x, pD.y, pD.z);
		++num;
	}
	meshHelper_renderImm(game3dContext, tess, uiMaterial);
}
void DrawUtils::drawImage(std::string filePath, Vec2& imagePos, Vec2& imageDimension, Vec2& idk, Vec2& idk2, bool system) {
	texturePtr = new TexturePtr();
	FilePath file(filePath, system);

	renderCtx->getTexture2(texturePtr, &file, false);
	...
	if (texturePtr != nullptr) {
		...
		renderCtx->flushImages(MC_Color(1.f, 1.f, 1.f), flushImageAddr, flushString);
		delete texturePtr;
	}
}
...
void DrawUtils::drawTextInWorld(std::string* textToSay, const Vec3& location, float tsize, Vec3i tColor, Vec3i bgColor, float opacity)
{
	Vec2 textPos;
	Vec4 rectPos;
...
void DrawUtils::drawNameTags(Entity* ent, float textSize, bool drawHealth, bool useUnicodeFont) {
	...
	if (refdef->OWorldToScreen(origin, ent->eyePos0.add(0, 0.5f, 0), textPos, fov, screenSize)) {
		...
		static auto nametagsMod = moduleMgr->getModule<NameTags>();
		int healthey = ent->Health;
		if (healthey == 6) healthey = 20;
		std::string health = " " + std::to_string(healthey);
		int popcounter = ent->popcount;
		static NameTags* nm = moduleMgr->getModule<NameTags>();
		if (nm->underline) {
			fillRectangle(Vec4(subRectPos.x, rectPos.y, subRectPos.z + getTextWidth(&std::string(std::to_string(healthey) + " "), textSize) + 5, subRectPos.w), blackColor, nametagsMod->opacity);
			fillRectangle(Vec4(subRectPos.x, subRectPos.y, subRectPos.z + getTextWidth(&std::string(std::to_string(healthey) + " "), textSize) + 5, subRectPos.w), MC_Color(100, 0, 255), 0.8f);
			if (popcounter >= 1 || popcounter <= -1) {
				fillRectangle(Vec4(subRectPos.x, rectPos.y, subRectPos.z + getTextWidth(&std::string(std::to_string(healthey) + " -" + std::to_string(popcounter)), textSize) + 5, subRectPos.w), blackColor, nametagsMod->opacity);
				fillRectangle(Vec4(subRectPos.x, subRectPos.y, subRectPos.z + getTextWidth(&std::string(std::to_string(healthey) + " -" + std::to_string(popcounter)), textSize) + 5, subRectPos.w), MC_Color(100, 0, 255), 0.8f);
			}
		}
		MC_Color Color = MC_Color(255, 255, 255);
		if (FriendsManager::findFriend(ent->getNameTag()->getText())) Color = MC_Color(0, 255, 0);
		drawText(textPos, &std::string(text), Color, textSize);
		drawText(Vec2(textPos.x + getTextWidth(&text, textSize), textPos.y), &health, MC_Color(0, 255, 0), textSize);
		if (popcounter == 1) drawText(Vec2(textPos.x + getTextWidth(&text, textSize) + getTextWidth(&std::string(std::to_string(healthey) + "   "), textSize), textPos.y), &std::string("-" + std::to_string(popcounter)), MC_Color(123, 0, 0), textSize);
		if (popcounter == 2) drawText(Vec2(textPos.x + getTextWidth(&text, textSize) + getTextWidth(&std::string(std::to_string(healthey) + "   "), textSize), textPos.y), &std::string("-" + std::to_string(popcounter)), MC_Color(0, 0, 255), textSize);
		if (popcounter == 3) drawText(Vec2(textPos.x + getTextWidth(&text, textSize) + getTextWidth(&std::string(std::to_string(healthey) + "   "), textSize), textPos.y), &std::string("-" + std::to_string(popcounter)), MC_Color(255, 0, 0), textSize);
		if (popcounter == 4) drawText(Vec2(textPos.x + getTextWidth(&text, textSize) + getTextWidth(&std::string(std::to_string(healthey) + "   "), textSize), textPos.y), &std::string("-" + std::to_string(popcounter)), MC_Color(0, 123, 255), textSize);
		if (popcounter == 5) drawText(Vec2(textPos.x + getTextWidth(&text, textSize) + getTextWidth(&std::string(std::to_string(healthey) + "   "), textSize), textPos.y), &std::string("-" + std::to_string(popcounter)), MC_Color(255, 0, 100), textSize);
		if (popcounter == 6) drawText(Vec2(textPos.x + getTextWidth(&text, textSize) + getTextWidth(&std::string(std::to_string(healthey) + "   "), textSize), textPos.y), &std::string("-" + std::to_string(popcounter)), MC_Color(0, 255, 100), textSize);
		if (popcounter >= 7) drawText(Vec2(textPos.x + getTextWidth(&text, textSize) + getTextWidth(&std::string(std::to_string(healthey) + "   "), textSize), textPos.y), &std::string("-" + std::to_string(popcounter)), MC_Color(100, 0, 255), textSize);
		...
		if (ent->getEntityTypeId() == 319 && nameTagsMod->displayArmor) {  // is player, show armor
			...
			float itemHeight = mcFont->getLineHeight() * scale;
			// armor
			for (int i = 0; i < 4; i++) {
				...
				if (stack->item != nullptr) {
					...
					DrawUtils::drawItem(stack, Vec2(x, y), 1.f, scale, false);
					//DrawUtils::drawText(vec2_t(x, y - itemHeight), &durastr, MC_Color(gamerColor), textSize * 0.55f, 1.f, true);
					DrawUtils::drawCenteredString(Vec2(x + textSize * 4.5f, y), &durastr, textSize * 0.6f, MC_Color(gamerColor), true);
					x += scale * spacing;
				}
			}
			// item
			{
				...
				if (stack->item != nullptr) {
					...
					if (!dura1 == 0) DrawUtils::drawCenteredString(Vec2(rectPos.z - textSize * 4.5f, y), &durastr, textSize * 0.6f, MC_Color(gamerColor), true);
					DrawUtils::drawItem(stack, Vec2(rectPos.z - 1.f - 15.f * scale, y), 1.f, scale, false);
				}
			}
		}
	}
...
void DrawUtils::drawFilledBox2D(Vec3 pos, float height)
{
	Vec3 myFill2D = Vec3(pos.x, pos.y, pos.z);
	float rentimer = 1;
	float zero = rentimer / 2;
	{
		Vec2 fill1 = DrawUtils::worldToScreen(Vec3(myFill2D).add(zero, zero, zero));
		Vec2 fill2 = DrawUtils::worldToScreen(Vec3(myFill2D).add(-zero, zero, zero));
		Vec2 fill3 = DrawUtils::worldToScreen(Vec3(myFill2D).add(-zero, zero, -zero));
		Vec2 fill4 = DrawUtils::worldToScreen(Vec3(myFill2D).add(zero, zero, -zero));
	...
	}
	{
		...
		Vec2 fill1 = DrawUtils::worldToScreen(Vec3(myFill2D).add(-zero, zero + height, -zero));
		Vec2 fill2 = DrawUtils::worldToScreen(Vec3(myFill2D).add(-zero, zero + height, zero));
		Vec2 fill3 = DrawUtils::worldToScreen(Vec3(myFill2D).add(zero, zero + height, zero));
		Vec2 fill4 = DrawUtils::worldToScreen(Vec3(myFill2D).add(zero, zero + height, -zero));
		if (!((fill1.x <= 0 && fill1.y <= 0) || (fill2.x <= 0 && fill2.y <= 0) || (fill3.x <= 0 && fill3.y <= 0) || (fill4.x <= 0 && fill4.y <= 0)))
			DrawUtils::drawQuad(Vec2(fill3), Vec2(fill4), Vec2(fill1), Vec2(fill2));
	}
	{
...
void DrawUtils::drawLineStrip3D(const std::vector<Vec3>& points, int mode) {
	if (game3dContext == 0 || entityFlatStaticMaterial == 0)
		return;
