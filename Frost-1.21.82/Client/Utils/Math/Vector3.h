...
static constexpr float RAD_DEG = 3.1415927f / 180.0f; // calculates on compile
...
struct Vector3 {
	...
	Vector3<T> Normalize() const { // Normalize the velocity
	...
	Vector3<T> floor() const { // std::floor for all positions 
		return Vector3<T>(std::floor(x), std::floor(y), std::floor(z));
	}

	Vector3<float> ToFloat() { // Changes a vector3<int> to float.
		return Vector3<float>((float)x, (float)y, (float)z);
	}

	Vector3<int> ToInt() { // Changes a vector3<float> to int.
		return Vector3<int>((int)x, (int)y, (int)z);
	}

	Vector3<T> add(Vector3<T> o) const { // add between vectors
		return Vector3<T>(x + o.x, y + o.y, z + o.z);
	}

	//returns the average of two Vector3
	Vector3 average(const Vector3& o) const {
		return Vector3((x + o.x) / 2, (y + o.y) / 2, (z + o.z) / 2);
	}
	...
	const static Vector2<float> CalcAngle(Vector3 ths, Vector3 dst) {
		...
		float yawDegrees = yaw * (180 / 3.1415927f);
		float pitch = atan2(deltaY, deltaXZ) * (180 / 3.1415927f);
