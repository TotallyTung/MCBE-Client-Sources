...
public:
	...
	static void Render() {
		...
		auto textWidth = ImRenderUtil::getTextWidth(&str, fontSize) + str.length() * spacing;
		auto textHeight = ImRenderUtil::getTextHeight(fontSize);
		{
			float posX = windowSize.x / 2 - textWidth / 2;
			for (auto c : str) {
				auto c2 = (std::string)"" + c;
				ImRenderUtil::drawText(Vector2<float>(posX, windowSize.y / 2 - textHeight / 2), &c2, UIColor(255, 255, 255), fontSize, 1.f);
				posX += ImRenderUtil::getTextWidth(&c2, fontSize) + spacing;
			}
		}
	};
