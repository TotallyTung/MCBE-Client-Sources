...
public:
	...
	inline static UIColor animateColor(UIColor endPoint, UIColor current, float speed) {
		return UIColor(
			animate(endPoint.r, current.r, speed) * 255.f,
			animate(endPoint.g, current.g, speed) * 255.f,
			animate(endPoint.b, current.b, speed) * 255.f,
			animate(endPoint.a, current.a, speed) * 255.f
		);
	}
	inline static ImColor animateColor(ImColor endPoint, ImColor current, float speed) {
		return ImColor(
			animate(endPoint.Value.x, current.Value.x, speed),
			animate(endPoint.Value.y, current.Value.y, speed),
			animate(endPoint.Value.z, current.Value.z, speed),
			animate(endPoint.Value.w, current.Value.w, speed)
		);
	}
